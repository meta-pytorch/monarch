...
impl FromStr for CarrierAddr {
    ...
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        ...
        Err("carrier address must use udp:// or unix://".into())
    }
}
        ...
        match query {
            Some(query) => {
                let pid = query
                    ...
                    .ok_or("parent locator only supports the authority query")?;
                Ok(Self::Pinned {
                    pid: parse_pid(pid)?,
                    address,
                })
...
async fn create_local_node(
    ...
    if is_root {
        ...
        println!("[node {rank} root task {}] {join_token}", task.id);
    } else {
        println!(
...
pub(crate) async fn worker(args: WorkerArgs) -> Result<()> {
    ...
    let parent_pid =
        match wait_for_parent(&node, Duration::from_secs(args.run.join_timeout_secs)).await {
            Ok(parent) => parent,
            Err(error) => {
                node.shutdown();
                node.join().await;
                remove_socket(unix_path.as_deref()).await?;
        ...
        while index < starting.len() {
            ...
            match tokio::fs::try_exists(&process.ready_file).await {
                Ok(true) => {
                    ...
                    remove_file(&process.ready_file).await?;
                    println!(
            ...
            if process.status.is_none() {
                process.status = Some(
                    process
                        ...
                        .wait()
                        .await
                        .with_context(|| format!("join worker node {}", process.rank))?,
                );
            }
            remove_file(&process.ready_file).await?;
        }
...
async fn run_root(node: Arc<Node>, args: &RunArgs, started: Instant) -> Result<()> {
    ...
    let result = json!({
        ...
        "echoes_per_second": latencies.len() as f64 / echo_elapsed.as_secs_f64(),
...
async fn wait_for_nodes(
    ...
    loop {
        ...
        if report_progress
            && (nodes.len() == expected
                || nodes.len() >= last_reported.saturating_add(progress_interval))
        {
        ...
        anyhow::ensure!(
            nodes.len() < expected,
            "node registry contains {} entries, expected exactly {expected}",
            nodes.len()
        );
        anyhow::ensure!(
...
async fn execute_experiment(
    ...
    let execution = async {
        ...
        started_at_ms = unix_millis()?;
        let connection_before = (selected.len() == 1).then(|| {
            let target = selected[0];
            let stats = node
                .transport()
                .connection_stats(target)
                .expect("warm-up establishes the measured connection");
            (target, stats)
        });
    ...
    let (completed, mean_latency, max_latency, error) = match execution {
        Ok(latencies) => {
            let mean_seconds =
                latencies.iter().map(Duration::as_secs_f64).sum::<f64>() / latencies.len() as f64;
            (
                ...
                latencies.iter().copied().max().unwrap_or_default(),
                None,
            )
        }
        Err(error) => (
...
async fn wait_for_entries(
    ...
    let mut last_reported: usize = 0;
    loop {
        if let Ok(entries) = node.enumerate(ResolveConsistency::Cached).await {
...
async fn run_all(
    ...
    while let Some(result) = tasks.join_next().await {
        ...
        if latencies.len() % progress_interval == 0 || latencies.len() == total {
            println!(
                "[root] completed {} {}/{}",
                kind.as_str(),
                latencies.len(),
...
fn spawn_operation(
    ...
    tasks.spawn(async move {
        ...
        let mut trailing = [0];
        anyhow::ensure!(
            recv.read(&mut trailing).await? == 0,
...
async fn send_payload(send: chrysalis::SendStream, size: usize) -> Result<chrysalis::SendStream> {
    let chunk = Bytes::from(vec![PAYLOAD_BYTE; OWNED_PAYLOAD_CHUNK_LEN]);
...
where
    W: AsyncWrite + Unpin,
{
    let chunk = [PAYLOAD_BYTE; PAYLOAD_CHUNK_LEN];
    let mut remaining = size;
    while remaining > 0 {
        let length = remaining.min(chunk.len());
    ...
    R: AsyncRead + Unpin,
{
    ...
    let mut chunk = [0; PAYLOAD_CHUNK_LEN];
    let mut remaining = size;
    while remaining > 0 {
        let length = remaining.min(chunk.len());
...
mod tests {
    ...
    fn worker_parent_tokens_round_trip_udp_and_unix_addresses() {
        ...
        assert_eq!(discovered.to_string().parse(), Ok(discovered));

        let pinned = ParentTarget::Pinned {
            pid: Pid::from_bytes([0x42; 16]),
            address: CarrierAddr::Unix("/tmp/chrysalis-head.sock".into()),
        };
        assert_eq!(pinned.to_string().parse(), Ok(pinned));
    }
