...
impl Admin {
    async fn connect(target: ConnectionTarget) -> Result<Self> {
        ...
        let ready = async {
            ...
            wait_for_root_registration(&store, DEFAULT_TIMEOUT).await
        }
        .await;
...
async fn wait_for_root_registration(store: &ExperimentStore, timeout: Duration) -> Result<()> {
    let deadline = Instant::now() + timeout;
    loop {
        if store.expected_nodes().await?.is_some() {
            return Ok(());
        }
        anyhow::ensure!(
            Instant::now() < deadline,
            "timed out synchronizing scale root registration"
        );
        tokio::time::sleep(POLL_INTERVAL).await;
    }
}
...
async fn add_experiment(store: &ExperimentStore, experiment: Experiment) -> Result<()> {
    anyhow::ensure!(
    ...
    if let ExperimentTargets::Count(count) = &experiment.targets {
    ...
    } else if let ExperimentTargets::Explicit(targets) = &experiment.targets {
    ...
    let status = wait_until_claimed(store, &experiment.name, DEFAULT_TIMEOUT).await?;
