...
static PAYLOAD_CHUNK: [u8; QUIC_CHUNK_SIZE] = [0x2a; QUIC_CHUNK_SIZE];
...
pub(crate) async fn run(args: NetworkBaselineArgs) -> Result<()> {
    ...
    match (args.protocol, same_hostname(&local_hostname, &hosts[0])) {
        (NetworkBaselineProtocol::Tcp, true) => {
            receive_tcp(
                receiver,
                args.bytes,
                args.connections,
        ...
        (NetworkBaselineProtocol::Tcp, false) => {
            send_tcp(
                receiver,
                args.bytes,
                args.connections,
        ...
        (
            protocol @ (NetworkBaselineProtocol::DirectQuic
            | NetworkBaselineProtocol::QuicAblation
            | NetworkBaselineProtocol::QuicIoAblation
            | NetworkBaselineProtocol::QuicWriteChunksAblation
            | NetworkBaselineProtocol::QuicSendWindowAblation
            | NetworkBaselineProtocol::QuicMtuAblation
            | NetworkBaselineProtocol::QuicPacingAblation
            | NetworkBaselineProtocol::QuicGsoAblation),
            true,
        ) => {
            anyhow::ensure!(
                args.connections == 1,
            ...
            receive_direct_quic(
                ...
                timeout,
                match protocol {
                    NetworkBaselineProtocol::DirectQuic => QuicFollowup::None,
                    NetworkBaselineProtocol::QuicAblation => QuicFollowup::Switch,
                    NetworkBaselineProtocol::QuicIoAblation => QuicFollowup::Io,
                    NetworkBaselineProtocol::QuicWriteChunksAblation => QuicFollowup::WriteChunks,
                    NetworkBaselineProtocol::QuicSendWindowAblation => QuicFollowup::SendWindow,
                    NetworkBaselineProtocol::QuicMtuAblation => QuicFollowup::Mtu,
                    NetworkBaselineProtocol::QuicPacingAblation => QuicFollowup::Pacing,
                    NetworkBaselineProtocol::QuicGsoAblation => QuicFollowup::Gso,
                    NetworkBaselineProtocol::Tcp => unreachable!("matched QUIC protocols"),
                },
                args.profile,
            )
            .await
...
async fn send_tcp(
    ...
    for (index, stream) in streams.into_iter().enumerate() {
        ...
        let stream_bytes = bytes / connections_u64 + u64::from(index < bytes % connections_u64);
...
async fn receive_direct_quic(
    ...
    let (mut control, _) = tokio::time::timeout(timeout, listener.accept())
        .await
    ...
    let protocol = match followup {
        QuicFollowup::Io => "direct-quic-copy-1",
        QuicFollowup::WriteChunks => "direct-quic-chunks-1",
        QuicFollowup::SendWindow => "direct-quic-batched-default-window-1",
        QuicFollowup::Mtu => "direct-quic-batched-path-mtu-1",
        QuicFollowup::Pacing => "direct-quic-batched-default-pacing-1",
        QuicFollowup::Gso => "direct-quic-batched-10-gso-1",
        QuicFollowup::None | QuicFollowup::Switch => "direct-quic",
    };
    ...
    match followup {
        ...
        QuicFollowup::Io => {
            for (protocol, mode) in [
                ("direct-quic-chunks-1", QuicIoMode::Chunks),
                ("direct-quic-chunks-2", QuicIoMode::Chunks),
                ("direct-quic-copy-2", QuicIoMode::Copy),
            ] {
        ...
        QuicFollowup::WriteChunks => {
            for (protocol, mode) in [
                ("direct-quic-batched-chunks-1", QuicIoMode::BatchedChunks),
                ("direct-quic-batched-chunks-2", QuicIoMode::BatchedChunks),
                ("direct-quic-chunks-2", QuicIoMode::Chunks),
            ] {
                receive_direct_io_phase(
                    &mut control,
        ...
        QuicFollowup::SendWindow => {
            for (protocol, send_window) in [
                ("direct-quic-batched-64m-window-1", Some(LARGE_SEND_WINDOW)),
                ("direct-quic-batched-64m-window-2", Some(LARGE_SEND_WINDOW)),
                ("direct-quic-batched-default-window-2", None),
            ] {
                receive_direct_io_phase(
                    &mut control,
                    address,
        ...
        QuicFollowup::Mtu => {
            for (protocol, max_udp_payload) in [
                ("direct-quic-batched-1200-mtu-1", Some(MIN_UDP_PAYLOAD)),
                ("direct-quic-batched-1200-mtu-2", Some(MIN_UDP_PAYLOAD)),
                ("direct-quic-batched-path-mtu-2", None),
            ] {
                receive_direct_io_phase(
                    &mut control,
                    address,
        ...
        QuicFollowup::Pacing => {
            for (protocol, disable_pacing) in [
                ("direct-quic-batched-pacing-disabled-1", true),
                ("direct-quic-batched-pacing-disabled-2", true),
                ("direct-quic-batched-default-pacing-2", false),
            ] {
                receive_direct_io_phase(
                    &mut control,
                    address,
        ...
        QuicFollowup::Gso => {
            for (protocol, max_transmit_segments) in [
                (
                    "direct-quic-batched-20-gso-1",
                    Some(EXPANDED_TRANSMIT_SEGMENTS),
                ),
                (
                    "direct-quic-batched-20-gso-2",
                    Some(EXPANDED_TRANSMIT_SEGMENTS),
                ),
                (
                    "direct-quic-batched-10-gso-2",
                    Some(BASELINE_TRANSMIT_SEGMENTS),
                ),
            ] {
                receive_direct_io_phase(
                    &mut control,
...
async fn receive_direct_io_phase(
    ...
    let (warmup_source, connection_before) =
        receive_quic_stream(&transport, QUIC_WARMUP_BYTES, timeout, mode).await?;
    ...
    let started = Instant::now();
    let (source, connection_after) =
        receive_quic_stream(&transport, expected, timeout, mode).await?;
    let elapsed = started.elapsed();
    let cpu = ProcessCpuTime::now().since(cpu_before);
...
async fn send_direct_io_phase(
    ...
    let connection_after =
        send_quic_stream(&transport, target, &destination, bytes, timeout, mode).await?;
    let elapsed = started.elapsed();
    let cpu = ProcessCpuTime::now().since(cpu_before);
    let io = transport.io_stats().since(io_before);
    let connection = connection_after.since(connection_before);
...
async fn send_quic_chunks(send: &mut chrysalis::SendStream, bytes: u64) -> Result<()> {
    let chunk = Bytes::from_static(&PAYLOAD_CHUNK);
    let mut remaining = bytes;
    while remaining > 0 {
        let count = usize::try_from(remaining.min(CHUNK_SIZE as u64)).expect("bounded chunk size");
