...
const SCHEMA: &str = "
    ...
    SELECT crsql_as_crr('experiments');
    ...
    SELECT crsql_as_crr('experiment_targets');
    ...
    CREATE TABLE IF NOT EXISTS results (
        ...
        echo_seconds REAL NOT NULL DEFAULT 0,
        echoes_per_second REAL NOT NULL DEFAULT 0,
        mean_echo_millis REAL NOT NULL DEFAULT 0,
        max_echo_millis REAL NOT NULL DEFAULT 0,
...
impl ExperimentStore {
    pub(crate) async fn open(path: &Path) -> Result<Self> {
        ...
        load_crsqlite(&connection, &extension)
            .with_context(|| format!("load cr-sqlite into {path_str}"))?;
        connection
            .execute_batch(SCHEMA)
            .await
            .with_context(|| format!("initialize experiment schema in {path_str}"))?;
        Ok(Self { connection })
    }

    pub(crate) async fn configure(&self, config: NodeConfig) -> Result<NodeConfig> {
        let replica = Replica::new(self.connection.clone())
            .await
            .context("initialize SQLite replica")?;
        Ok(ReplicationTopology::new(replica).configure(config))
    }
    ...
    pub(crate) async fn expected_nodes(&self) -> Result<Option<usize>> {
        let mut rows = self
            .connection
            .query(
                "SELECT COALESCE(MIN(expected_nodes), 0), \
                        COALESCE(MAX(expected_nodes), 0) \
                 FROM nodes",
                (),
            )
            .await
        ...
        let row = rows
            .next()
            .await
            .context("read expected node count")?
            .expect("aggregate query returns one row");
        let minimum: i64 = row.get(0)?;
        let maximum: i64 = row.get(1)?;
        if minimum != maximum || maximum == 0 {
            return Ok(None);
        }
        Ok(Some(
            usize::try_from(maximum).context("expected node count is invalid")?,
        ))
    }
                ...
                transaction
                    .execute(
                        ...
                        vec![
                            Value::Text(experiment.name.clone()),
                            Value::Integer(
                                i64::try_from(position).context("target position exceeds i64")?,
                            ),
    ...
    pub(crate) async fn claim_experiment(
        ...
        assert_eq!(updated, 1, "selected pending experiment disappeared");
        transaction
            .commit()
            .await
    ...
    pub(crate) async fn record_result(&self, result: ExperimentResult) -> Result<()> {
        ...
        let echoes_per_second = if result.elapsed.is_zero() {
            0.0
        } else {
        ...
        transaction
            .execute(
                "INSERT INTO results (\
                    ...
                    started_at_ms, finished_at_ms, warmup_seconds, echo_seconds, \
                    echoes_per_second, mean_echo_millis, max_echo_millis, \
...
fn extension_path() -> Result<PathBuf> {
    match std::env::var_os("CHRYSALIS_CRSQLITE_PATH") {
        Some(path) => Ok(path.into()),
        None => packaged_extension_path(),
    }
}

#[cfg(fbcode_build)]
fn packaged_extension_path() -> Result<PathBuf> {
    Ok(buck_resources::get("monarch/chrysalis-scale/crsqlite")?.to_path_buf())
}

#[cfg(not(fbcode_build))]
fn packaged_extension_path() -> Result<PathBuf> {
    anyhow::bail!("CHRYSALIS_CRSQLITE_PATH is required outside fbcode")
}
...
mod tests {
    ...
    #[tokio::test]
    async fn conflicting_expected_counts_are_not_yet_converged() {
        let directory = tempfile::tempdir().expect("create temporary directory");
