...
const CRSQLITE_TARGET: &str =
    "fbsource//third-party/libsql/main/src/libsql-sqlite3/ext/crr:crsql_bundle[cdylib]";
...
const PACKAGE_CRSQLITE: &str = "crsqlite.so";
...
fn build_package(expire: &str) -> Result<String> {
    ...
    let crsqlite = output_path(CRSQLITE_TARGET)
        .context("buck output did not contain the cr-sqlite extension")?;
    let root = PathBuf::from(
        command_output(Command::new("buck").args(["root", "--kind", "project"]))?.trim(),
    );
    let binary = resolve_build_output(&root, binary);
    let crsqlite = resolve_build_output(&root, crsqlite);
    ...
    fs::copy(&crsqlite, staging.path().join(PACKAGE_CRSQLITE))
        .with_context(|| format!("stage cr-sqlite extension from {}", crsqlite.display()))?;
    ...
    let package_config = json!({
        "paths": [PACKAGE_BINARY, PACKAGE_CRSQLITE],
        "build_command": "",
    });
    fs::write(
...
fn make_job_spec(
    ...
    let crsqlite = format!("/packages/{package_name}/{PACKAGE_CRSQLITE}");
        ...
        "hpcTaskGroups": [{
            ...
            "spec": {
                ...
                "env": {
                    ...
                    "CHRYSALIS_CRSQLITE_PATH": crsqlite,
                    "RUST_BACKTRACE": "1",
                },
                "resourceLimit": {
                    "ramMB": args.ram_mb,
...
mod tests {
    ...
    fn job_spec_reserves_one_host_per_task_and_preserves_benchmark_parameters() {
        ...
        assert_eq!(
            group["spec"]["env"]["CHRYSALIS_CRSQLITE_PATH"],
            "/packages/pkg/crsqlite.so"
        );
    ...
    fn relative_build_outputs_are_resolved_from_the_project_root() {
        ...
        assert_eq!(
            ...
            Path::new("/project/relative/artifact")
        );
        assert_eq!(
