...
impl PyRdmaManager {
    ...
    fn create_rdma_manager_nonblocking<'py>(
        ...
        PyPythonTask::new(async move {
            // Note: RDMA support check removed from RdmaController's node
            // The check should happen on the target proc_mesh nodes where RdmaManagerActor is spawned
            // This allows RdmaController to run on any node while RDMA operations occur on appropriate hardware
