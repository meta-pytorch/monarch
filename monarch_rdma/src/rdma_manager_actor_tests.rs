...
mod tests {
    ...
    /// Tests that read_into works on same-actor loopback via the memcpy fallback.
    ...
    #[timed_test::async_timed_test(timeout_secs = 60)]
    async fn test_rdma_read_into_same_actor_loopback() -> Result<(), anyhow::Error> {
    ...
    /// Tests that write_from works on same-actor loopback via the memcpy fallback.
    #[timed_test::async_timed_test(timeout_secs = 60)]
    async fn test_rdma_write_from_same_actor_loopback() -> Result<(), anyhow::Error> {
        const BSIZE: usize = 256;
        let devices = get_all_devices();
        if devices.is_empty() {
            println!("Skipping test: RDMA devices not available");
