...
//! Reliable-connection queue pair: [`RCQueuePair`] on plain ibverbs, the RC
//! `connect` transition helper, and [`RCQueuePairActor`] that drives one.
...
/// [`crate::backend::ibverbs::manager_actor::IbvManagerActor`]. The same conceptual
...
pub(crate) struct QpKey {
    pub(crate) self_device: String,
    pub(crate) other_id: ActorId,
    pub(crate) other_device: String,
}
...
// `RCQueuePairActor` that owns and operates the local QP; the peer's
...
// peer later wants to RDMA us, it spawns its own `RCQueuePairActor`
...
impl<M: Manager, Qp: IbvQueuePair> RCQueuePairActor<M, Qp> {
    pub(crate) fn new(
        qp_key: QpKey,
        local_manager: ActorRef<M>,
        peer_manager: ActorRef<M>,
...
#[async_trait]
impl<M: Manager, Qp: IbvQueuePair> Actor for RCQueuePairActor<M, Qp> {
    async fn init(&mut self, this: &Instance<Self>) -> Result<(), anyhow::Error> {
        let local_info = self.qp.get_qp_info().map_err(|e| {
            tracing::error!(qp_key = ?self.qp_key, error = %e, "RCQueuePairActor init: get_qp_info failed");
            ...
            match tokio::time::timeout(self.init_timeout, rx.recv()).await {
                ...
                Ok(Err(e)) => {
                    tracing::error!(
                        ...
                        peer_manager = ?self.peer_manager,
                        error = %e,
                        "RCQueuePairActor init: peer reply port closed",
                    );
                ...
                Err(_) => {
                    tracing::error!(
                        ...
                        "RCQueuePairActor init: timed out waiting for peer reply",
                    );
                    return Err(anyhow::anyhow!(
        ...
        self.qp.connect(&peer_info).map_err(|e| {
            tracing::error!(
                ...
                peer_info = ?peer_info,
                error = %e,
                "RCQueuePairActor init: connect failed",
            );
...
#[async_trait]
impl<M: Manager, Qp: IbvQueuePair> Handler<ProcessOps<M>> for RCQueuePairActor<M, Qp> {
    async fn handle(
        &mut self,
        cx: &Context<Self>,
        msg: ProcessOps<M>,
...
#[async_trait]
impl<M: Manager, Qp: IbvQueuePair> Handler<Tick> for RCQueuePairActor<M, Qp> {
...
mod tests {
    ...
    use crate::backend::ibverbs::primitives::IbvConfig;
    use crate::backend::ibverbs::queue_pair::legacy;
    ...
    /// Mock manager used by `RCQueuePairActor` tests.
    ///
    /// Plays two roles:
    ...
    /// Local message that spawns a `RCQueuePairActor` as a child of
    ...
    struct SpawnQpaChild {
        ...
        max_send_wr: u32,
        max_rd_atomic: u32,
        reply: hyperactor::OncePortHandle<ActorHandle<RCQueuePairActor<QpaMockManager, MockQp>>>,
    }
    ...
    impl Handler<SpawnQpaChild> for QpaMockManager {
        async fn handle(&mut self, cx: &Context<Self>, msg: SpawnQpaChild) -> Result<()> {
            ...
            let actor = RCQueuePairActor::new(
                msg.qp_key,
                local_manager,
                msg.peer_manager,
                msg.qp,
    ...
    /// `IbvQueuePair` mock used by `RCQueuePairActor` tests. Cloning is
    ...
    impl QpaHarness {
        ...
        async fn spawn_actor(
            ...
            qp: MockQp,
            is_loopback: bool,
        ) -> Result<ActorHandle<RCQueuePairActor<QpaMockManager, MockQp>>> {
    ...
    async fn await_status(
        handle: &ActorHandle<RCQueuePairActor<QpaMockManager, MockQp>>,
    ...
    // =================================================================
    // RCQueuePairActor op processing
    // =================================================================
    ...
    impl QpaHarness {
        /// Spawn a `RCQueuePairActor` in loopback mode (no peer round
        ...
        async fn spawn_ready_actor(
            ...
            ActorHandle<RCQueuePairActor<QpaMockManager, MockQp>>,
            MockQp,
    ...
    fn submit_ops(
        harness: &QpaHarness,
        actor: &ActorHandle<RCQueuePairActor<QpaMockManager, MockQp>>,
