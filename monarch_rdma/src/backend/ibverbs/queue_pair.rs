...
use super::manager_actor::IbvManagerLocalMessage;
...
// observe its own.
//
// Progress is tracked by `state: InitState`. The qp is held in a
// `QpGuard` so any failure path (or aborted message delivery)
// destroys it; the success path hands the guard to the manager
// via `qp_initializer_done`.
...
///
/// `pub` to satisfy `IbvManagerLocalMessage`'s derived `pub` traits.
...
/// Stages of the QP setup handshake. See module docs.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[cfg_attr(not(test), allow(dead_code))]
enum InitState {
    /// Initial state after `init`: waiting for both `PeerInfo` and
    /// the peer's `NotifyRts`.
    AwaitingPeerInfoAndPeerRts,
    /// The peer's `NotifyRts` has arrived; only our `PeerInfo` is
    /// missing.
    AwaitingPeerInfo,
    /// `PeerInfo` arrived and `qp.connect` succeeded; only the
    /// peer's `NotifyRts` is missing.
    AwaitingPeerRts,
    /// Terminal: `qp_initializer_done` has been sent to the owner.
    Succeeded,
    /// Terminal: `qp_initializer_failed` has been sent to the owner.
    Failed,
}
...
pub(super) trait QpOwner:
    ...
    + Handler<IbvManagerLocalMessage>
{
}
...
pub(super) struct QueuePairInitializer<A: QpOwner> {
    ...
    /// via `qp_initializer_done`) or fails (dropped here, which
    ...
    A: QpOwner,
{
    pub(super) fn new(
        ...
        Self {
            ...
            timeout,
            state: InitState::AwaitingPeerInfoAndPeerRts,
            timeout_handle: None,
        }
    }
    ...
    /// Transition to `Failed`, drop the qp guard (destroying any
    /// qp held), and report failure to the owning manager.
    fn fail(&mut self, this: &Instance<Self>, error: String) -> Result<(), anyhow::Error> {
        ...
        self.owner.send(
            this,
            IbvManagerLocalMessage::QpInitializerFailed {
    ...
    /// Transition to `Succeeded` and hand the qp guard to the owning
    /// manager via `qp_initializer_done`.
    fn done(&mut self, this: &Instance<Self>) -> Result<(), anyhow::Error> {
        ...
        self.owner.send(
            this,
            IbvManagerLocalMessage::QpInitializerDone {
                qp_key: self.qp_key.clone(),
                qp,
            },
        )?;
    ...
    /// [`Self::fail`] on error; the caller decides which terminal
    /// arm to invoke.
    ...
    A: QpOwner,
{
    ...
    async fn handle_undeliverable_message(
        ...
        match self.state {
            InitState::Succeeded | InitState::Failed => {
                tracing::warn!(
            ...
            InitState::AwaitingPeerInfoAndPeerRts
            | InitState::AwaitingPeerInfo
            | InitState::AwaitingPeerRts => {
    ...
    async fn handle(&mut self, cx: &Context<Self>, msg: PeerInfo) -> Result<(), anyhow::Error> {
        match self.state {
            InitState::AwaitingPeerInfoAndPeerRts => {
                ...
                self.state = InitState::AwaitingPeerRts;
                self.arm_timeout(cx);
                Ok(())
            }
            InitState::AwaitingPeerInfo => {
            ...
            InitState::AwaitingPeerRts => unreachable!("duplicate PeerInfo in AwaitingPeerRts"),
            InitState::Succeeded => unreachable!("PeerInfo received in Succeeded state"),
            InitState::Failed => {
                tracing::warn!("PeerInfo received after queue pair already failed");
                Ok(())
            }
        }
    }
    ...
    A: QpOwner,
{
    async fn handle(&mut self, cx: &Context<Self>, _msg: NotifyRts) -> Result<(), anyhow::Error> {
        match self.state {
            InitState::AwaitingPeerInfoAndPeerRts => {
                self.state = InitState::AwaitingPeerInfo;
                self.arm_timeout(cx);
                Ok(())
            }
            InitState::AwaitingPeerRts => self.done(cx),
            InitState::AwaitingPeerInfo => unreachable!("duplicate NotifyRts in AwaitingPeerInfo"),
            InitState::Succeeded => unreachable!("NotifyRts received in Succeeded state"),
            InitState::Failed => {
                tracing::warn!("NotifyRts received after queue pair already failed");
                Ok(())
            }
        }
    }
    ...
    A: QpOwner,
{
    async fn handle(
        ...
        match self.state {
            InitState::AwaitingPeerInfoAndPeerRts
            | InitState::AwaitingPeerInfo
            | InitState::AwaitingPeerRts => self.fail(cx, "QP initialization timed out".into()),
            InitState::Succeeded | InitState::Failed => Ok(()),
        }
    }
}
...
mod tests {
    ...
    use crate::backend::ibverbs::manager_actor::IbvManagerLocalMessage;
    ...
    #[async_trait]
    impl Handler<IbvManagerLocalMessage> for MockManager {
        async fn handle(&mut self, _cx: &Context<Self>, msg: IbvManagerLocalMessage) -> Result<()> {
            match msg {
                IbvManagerLocalMessage::QpInitializerDone { qp_key, qp } => {
                    let _ = qp.into_inner();
                    self.state.lock().unwrap().done.push(qp_key);
                }
                IbvManagerLocalMessage::QpInitializerFailed { qp_key, error } => {
                    self.state.lock().unwrap().failed.push((qp_key, error));
                }
                _ => panic!("MockManager only handles QpInitializerDone/Failed"),
            }
            Ok(())
        }
    }
    ...
    /// initializer's well-known port to drive it to `Succeeded`.
    #[tokio::test]
    ...
    /// transitions to `AwaitingPeerRts`, but the test never delivers
    /// `NotifyRts` back to the initializer's port. The rearmed timer
    /// fires and the handshake is reported as `Failed`.
    #[tokio::test]
    async fn test_notify_rts_timeout_after_peer_info() -> Result<()> {
        ...
        // ran `qp.connect` + sent NotifyRts to the peer and parked in
        // `AwaitingPeerRts` before the rearmed timer fired.
    ...
    /// from `AwaitingPeerRts` and `handle_undeliverable_message`
    /// trips `fail()`.
    #[tokio::test]
    ...
    /// Once the initializer is in `Failed`, a late undeliverable is
