...
//! lost-wakeup prevention. Notifications cover empty-to-nonempty transitions,
//! full-to-writable transitions, and closure.
...
impl<T> Receiver<T> {
    ...
    pub(crate) fn try_pop(&self) -> Option<T> {
        let mut state = self
            ...
            .lock()
            .expect("queue mutex should not be poisoned");
        let notify = state.values.len() == self.inner.capacity;
        let value = state.values.pop_front();
        drop(state);
        if notify && value.is_some() {
            self.inner.notifier.notify();
        }
        value
    }
...
mod tests {
    ...
    #[test]
    fn popping_from_full_queue_notifies_blocked_producer() {
        let notifier = Arc::new(CountingNotifier::default());
        let (sender, receiver) = bounded(
            NonZeroUsize::new(1).expect("test capacity should be nonzero"),
            notifier.clone(),
        );
        sender.try_push(1).expect("fill queue");
