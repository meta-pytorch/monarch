...
mod tests {
    ...
    fn receive_completion_returns_appended_bytes_and_original_allocation() {
        ...
        let (_, _, mut buffer, _, cancellation) = submission.into_parts();
        assert!(cancellation.complete());
        buffer.writable()[..4].copy_from_slice(b"data");
        buffer.truncate(initial_len + 4);
        let completion =
    ...
    fn dropping_completion_queue_releases_posted_receive_budget() {
        ...
        let (_, _, buffer, _, cancellation) = submission.into_parts();
        assert!(cancellation.complete());
