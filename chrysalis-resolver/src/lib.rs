...
pub enum ResolveError {
    ...
    /// The MAST status command returned a JSON object with an invalid shape.
    #[error("MAST status for {job} is malformed: {reason}")]
    MalformedResponse { job: String, reason: &'static str },
...
fn resolve_mast_status(job: &str, status: &Value, port: u16) -> Result<Resolution, ResolveError> {
    let status = status
        .as_object()
        .ok_or_else(|| ResolveError::MalformedResponse {
            job: job.to_owned(),
    ...
    let state = status.get("state").and_then(Value::as_str).ok_or_else(|| {
        ResolveError::MalformedResponse {
            job: job.to_owned(),
            reason: "state is missing or is not a string",
        }
    })?;
    if matches!(state, "DEAD" | "FAILED" | "COMPLETE") {
    ...
    let groups = status
        ...
        .ok_or_else(|| ResolveError::MalformedResponse {
            job: job.to_owned(),
            reason: "missing taskGroups",
        })?
        .as_array()
        .ok_or_else(|| ResolveError::MalformedResponse {
            job: job.to_owned(),
            reason: "taskGroups is not an array",
        })?;
    let group = groups
        .iter()
    ...
    let tasks = group["tasks"]
        ...
        .ok_or_else(|| ResolveError::MalformedResponse {
            job: job.to_owned(),
            reason: "nodes.tasks is not an array",
        })?;
    let root_ip = tasks
        .iter()
        .filter(|task| {
            !matches!(
                task["state"].as_str(),
                Some("DEAD" | "FAILED" | "COMPLETE" | "STOPPED")
            )
        })
        .filter_map(|task| {
            ...
            let ordinal = task["taskIndex"]
                ...
                .unwrap_or(u64::MAX);
            let hostname = task["hostname"].as_str().unwrap_or("");
            Some((ordinal, hostname, ip))
        })
        .min()
        .map(|(_, _, ip)| ip)
