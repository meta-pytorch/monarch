...
impl FromStr for PidPrefix {
    ...
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        ...
        if !value.bytes().all(|digit| digit.is_ascii_hexdigit()) {
            return Err(ParsePidPrefixError::InvalidHex);
        }
        ...
        for (index, digit) in value.bytes().enumerate() {
            let nibble = match digit {
                ...
                b'A'..=b'F' => digit - b'A' + 10,
                _ => unreachable!("PID prefix was checked as ASCII hexadecimal"),
            };
            if index % 2 == 0 {
...
pub fn target_pid(datagram: &[u8]) -> Option<Pid> {
    ...
    let start = if first & 0x80 == 0 {
        ...
        if first & 0x40 == 0 || datagram.len() < 1 + CID_LEN {
            return None;
        }
        1
    } else {
...
mod tests {
    ...
    fn pid_prefixes_reject_empty_nonhex_and_oversized_text() {
        ...
        assert_eq!(
            "🙂".repeat(PID_LEN).parse::<PidPrefix>(),
            Err(ParsePidPrefixError::InvalidHex)
        );
    }
    ...
    #[test]
    fn rejects_malformed_short_headers() {
        let cid = RoutedCid::issued(PID, ConnectionKey::from_u32(1));
        let mut truncated = vec![0x40];
        truncated.extend_from_slice(PID.as_bytes());
        let mut missing_fixed_bit = vec![0x00];
        missing_fixed_bit.extend_from_slice(cid.as_bytes());

        assert_eq!(target_pid(&truncated), None);
        assert_eq!(target_pid(&missing_fixed_bit), None);
    }
