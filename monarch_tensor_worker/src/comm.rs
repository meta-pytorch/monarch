...
impl CommBackend {
    pub fn new(
        ...
        let proc = Proc::local();
        let (instance, _) = proc.instance(&format!("comm_backend_{rank}")).unwrap();
        Self {
            instance,
            comm,
            rank,
...
mod tests {
    ...
    async fn group_send_recv() {
        ...
        let (handle0, handle1) = tokio::join!(handle0, handle1);
        let ((_client0, handle0), (_client1, handle1)) = (handle0.unwrap(), handle1.unwrap());
    ...
    async fn test_comm_work() -> Result<()> {
        ...
        let (handle0, handle1) = tokio::join!(handle0, handle1);
        let ((_client0, handle0), (_client1, handle1)) = (handle0?, handle1?);
