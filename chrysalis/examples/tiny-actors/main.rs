...
// creates the mutually trusted ephemeral QUIC identities used by
// Alice and Bob.
mod identity;
...
async fn main() {
    ...
    // her child. Both nodes share one in-memory carrier and mutually
    // trust each other.
    ...
    let mut identities = identity::mutually_trusted_identities(2).into_iter();
    ...
    println!("tiny-actors, checkpoint 4: Echo runs behind a Tokio mailbox");
    println!(
        "Echo response crossed the mailbox: {}",
        echo_response == ECHO_PAYLOAD
    );
