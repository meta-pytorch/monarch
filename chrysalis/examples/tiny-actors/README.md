...
4. **Confirm Echo's mailbox boundary.** The checkpoint 3 call already passes
   through a sender-only handle. A separate Tokio task receives the request,
   performs the actor operation, and replies through a one-shot channel;
   checkpoint 4 reports that path explicitly.
...
- `identity.rs` creates mutually trusted ephemeral identities for Alice and
  Bob.
