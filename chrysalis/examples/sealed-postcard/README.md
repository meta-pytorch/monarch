...
Each node receives an ephemeral certificate and private key. The certificate
determines its 128-bit PID, and the three generated TLS configurations trust one
another. These identities exist only for this run.
...
5. **Observe Relay forwarding.** Relay's carrier is wrapped by `ObservedSocket`,
   which records raw ingress and egress datagrams without changing them. The
   program finds a Bob-targeted datagram received from Alice and the exact same
   bytes sent onward to Bob. Relay can read the destination PID from the QUIC
   connection ID, but the literal postcard does not appear in the recorded
   datagram bytes.
...
- `identity.rs` creates mutually trusted ephemeral test identities.
