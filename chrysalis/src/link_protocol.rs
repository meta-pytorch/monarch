...
const SESSION_REJECTED_ERROR_CODE: u64 = 1;
        ...
        tokio::select! {
            ...
            incoming = protocol.accept() => {
                let incoming = match incoming {
                    ...
                    Err(error) => {
                        error!(protocol = ?registration.id, error = %error, "link protocol accept failed");
                        break;
                    }
                };
            ...
            classified = classifiers.join_next(), if !classifiers.is_empty() => {
                ...
                let classified = match classified {
                    ...
                    Err(error) => {
                        error!(error = %error, "link session classifier task failed");
                        continue;
                    }
                };
                let Some((peer, link, mut stream)) = classified else {
                    continue;
                };
                ...
                if !admitted || active_children.contains_key(&link) {
                    debug!(
                        ...
                        link = ?link,
                        admitted,
                        duplicate = active_children.contains_key(&link),
                        "rejecting link protocol session"
                    );
                    rejections.spawn(reject_stream(stream));
                    continue;
                }
                let handler = registration.handler.clone();
                let abort = child_sessions.spawn(async move {
                    if stream.send_mut().write_all(&[SESSION_ACK]).await.is_ok() {
            ...
            _ = rejections.join_next(), if !rejections.is_empty() => {}
            completed = parent_session_completed(&mut parent_session), if parent_session.is_some() => {
                let (link, result) = completed;
                parent_session = None;
                parent_attempted = None;
                if let Err(error) = result
                    && !error.is_cancelled()
                {
                    error!(protocol = ?registration.id, link = ?link, error = %error, "parent link protocol session failed");
                }
                if let Some(parent) = &parent {
                    tokio::time::sleep(parent.retry_delay).await;
                }
                reconcile_parent(
                    &registration,
                    &protocol,
...
async fn parent_session_completed(
    session: &mut Option<(LinkId, JoinHandle<()>)>,
) -> (LinkId, Result<(), JoinError>) {
    let (link, task) = session
        .as_mut()
        .expect("guarded parent session should be present");
    (*link, task.await)
}

async fn reject_stream(stream: Stream) {
    let (send, receive) = stream.into_parts();
    let _ = tokio::join!(
        send.reset(SESSION_REJECTED_ERROR_CODE),
        receive.stop(SESSION_REJECTED_ERROR_CODE),
    );
}
...
async fn classify(mut incoming: IncomingStream) -> Option<(Pid, LinkId, Stream)> {
    ...
    match tokio::time::timeout(
        ...
        incoming.stream_mut().recv_mut().read_exact(&mut bytes),
    )
    .await
    {
        Ok(Ok(_)) => {}
        Ok(Err(error)) => {
            debug!(peer = ?peer, error = %error, "rejecting link session identifier read failure");
            return None;
        }
        Err(_) => {
            debug!(peer = ?peer, "rejecting link session identifier timeout");
            return None;
        }
    }
...
fn fence_removed_children(
    ...
    let links = links.values().copied().collect::<HashSet<_>>();
