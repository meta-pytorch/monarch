...
use crate::protocol::MAX_ENUMERATION_PAGE_SIZE;
...
pub enum CodecError {
    ...
    /// The declared frame body cannot contain the framing envelope.
    #[error("frame body is too small: {len} bytes, minimum {min}")]
    ...
    /// An enumeration request exceeds the maximum page size.
    #[error("enumeration limit is too large: {limit}, maximum {max}")]
    EnumerationLimit { limit: u32, max: u32 },
...
fn decode_message(tag: u8, decoder: &mut Decoder<'_>) -> Result<Message, CodecError> {
    match tag {
        ...
        ENUMERATE => {
            let request = RequestId::from_u64(decoder.u64()?);
            let consistency = decode_resolve_consistency(decoder.u8()?)?;
            let cursor = decoder.optional_cursor()?;
            let limit = decoder.u32()?;
            check_enumeration_limit(limit)?;
...
fn check_enumeration_limit(limit: u32) -> Result<(), CodecError> {
    if limit > MAX_ENUMERATION_PAGE_SIZE {
        Err(CodecError::EnumerationLimit {
            limit,
            max: MAX_ENUMERATION_PAGE_SIZE,
        })
    } else {
        Ok(())
    }
...
mod tests {
    ...
    #[test]
    fn rejects_undersized_frame_prefix_before_allocation() {
        assert_eq!(
            frame_body_len(1_u32.to_be_bytes()),
    ...
    #[test]
    fn rejects_oversized_enumeration_limit() {
        let message = Message::Enumerate {
            request: RequestId::from_u64(1),
            consistency: ResolveConsistency::Cached,
            cursor: None,
            limit: MAX_ENUMERATION_PAGE_SIZE + 1,
        };
        assert_eq!(
            encode_frame(&message),
            Err(CodecError::EnumerationLimit {
        ...
        let mut frame = encode_frame(&Message::Enumerate {
            request: RequestId::from_u64(1),
            consistency: ResolveConsistency::Cached,
            cursor: None,
            limit: MAX_ENUMERATION_PAGE_SIZE,
        })
        .expect("encode bounded enumeration request");
        let limit = frame.len() - 4;
        frame[limit..].copy_from_slice(&(MAX_ENUMERATION_PAGE_SIZE + 1).to_be_bytes());
        assert_eq!(
            decode_frame(&frame),
            Err(CodecError::EnumerationLimit {
                limit: MAX_ENUMERATION_PAGE_SIZE + 1,
