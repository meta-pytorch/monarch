...
async fn export_snapshot(
    ...
    exported.extend(
        service
            ...
            .await
            .into_iter()
            .filter_map(|entry| route_through_local(entry, local_entry)),
    );
    exported
}

fn route_through_local(entry: ProcEntry, local_entry: &ProcEntry) -> Option<(Pid, ProcEntry)> {
    if local_entry.locators.is_empty() {
        return None;
    }
    let entry = ProcEntry {
        pid: entry.pid,
...
mod tests {
    ...
    #[test]
    fn descendants_are_not_exported_without_a_local_route() {
        let mut local = entry(MIDDLE, 20);
        local.locators.clear();

        assert_eq!(route_through_local(entry(LEAF, 30), &local), None);
    }
