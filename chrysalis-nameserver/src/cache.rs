...
pub struct ResolverCache {
    ...
    revision_floors: BTreeMap<Pid, u64>,
}
...
impl ResolverCache {
    ...
    pub fn update(&mut self, result: Resolution, now: CacheTime) -> Result<bool, CacheError> {
        ...
        if let Some(current) = self.revision_floors.get(&pid) {
            if revision.value < *current {
                return Ok(false);
            }
            if revision.value == *current {
                let Some(cached) = self.entries.get(&pid) else {
                    return Ok(false);
                };
                if same_result(&result, &cached.result) {
                    return Ok(false);
                }
        ...
        self.revision_floors.insert(pid, revision.value);
        Ok(true)
    }

    /// Returns a currently usable cached result.
    pub fn resolve(&mut self, pid: Pid, now: CacheTime) -> Option<Resolution> {
        if self
            .entries
            .get(&pid)?
            .expires_at
        ...
        self.entries.get(&pid).map(|cached| cached.result.clone())
    }

    /// Clears all values and revision floors when the parent link closes.
    pub fn clear(&mut self) {
        self.entries.clear();
        self.revision_floors.clear();
    }
}

fn same_result(left: &Resolution, right: &Resolution) -> bool {
    match (left, right) {
        (
            Resolution::NotFound {
                pid: left_pid,
        ...
        ) => left_pid == right_pid && left_revision == right_revision,
        _ => left == right,
    }
}
...
mod tests {
    ...
    fn negative_expiry_is_receiver_relative_and_replay_does_not_extend_it() {
        ...
        assert!(
            !cache
                .update(not_found(1, 5_000), CacheTime::from_millis(140))
                .expect("replay equivalent negative result")
        );
        assert_eq!(cache.resolve(TARGET, CacheTime::from_millis(150)), None);

        assert!(
            !cache
                .update(negative, CacheTime::from_millis(150))
                .expect("ignore replay after expiry")
        );
