    ...
    fn complete_admit(
        ...
        if effects != ApplyEffects::default() {
            return self.fail_active(
                version,
                SessionError::UnexpectedEffects("link admission produced observable effects"),
            );
        }
        if self.close_requested {
    ...
    fn complete_publication(
        ...
        let effects = match result {
            ...
            Err(error) => {
                ...
                return self.fail_active(version, SessionError::Apply(error));
            }
        };
            ...
            && change.link != self.link
        {
            return self.fail_active(
                version,
                SessionError::UnexpectedEffects("directory change belongs to another link"),
            );
        }
        if matches!(reply, PublicationReply::None) && effects.directory_change.is_some() {
            return self.fail_active(
                version,
                SessionError::UnexpectedEffects("staging command changed the visible directory"),
            );
        }
        ...
        let response = match response {
            ...
            Err(error) => {
                return self.fail_active(version, error);
            }
        };
        if self.close_requested {
            return Ok(Some(self.begin_remove()));
