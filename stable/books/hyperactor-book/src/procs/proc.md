...
the outside world only through a [`Gateway`](gateway.html), its connectivity
...
Useful in tests.
- `Proc::configured(proc_id, forwarder)` -- wraps `Gateway::configured(location, forwarder)` so the proc advertises `location` and uses `forwarder` for
egress.
...
uid and forward to the child gateway peer. See [ProcId](../references/proc_id.html)
for identity details and [Addr](../references/reference.html) for addressable
references.
...
[routing decision](gateway.html#the-single-routing-decision): deliver to a local
...
[RemoteSpawn](../actors/remote_spawn.html) for spawning details.

## See Also

- [Gateway](gateway.html) -- the connectivity boundary every proc attaches to
- [Host](host.html) -- how hosts create and manage procs
- [ProcId](../references/proc_id.html) -- proc addressing
