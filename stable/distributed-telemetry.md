...
telemetry is the query layer behind the [Monarch Dashboard](monarch-dashboard.html)
and complements the live [Mesh Admin TUI](admin-tui.html).

Start with the [observability overview](observability.html) if you are choosing a
...
snapshots. The default is `0` (disabled). When `include_dashboard=True`, a value
of `0` is automatically changed to 30 seconds because the dashboard requires
snapshot data. Snapshots and event telemetry describe different views of the
same job:
...
[Mesh Admin TUI diagnostics](admin-tui.html) to check that host.
...
- [Observability overview](observability.html)
- [Monarch Dashboard](monarch-dashboard.html)
- [Mesh Admin TUI](admin-tui.html)
- [OpenTelemetry and Grafana](generated/examples/otel_collector.html)
- [Jobs API](api/monarch.job.html)