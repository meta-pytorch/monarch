...
- **[Stream forwarders](stream-forwarders.html)** -- where lines are framed and posted to the appender
- **[Client actor](client.html)** -- aggregation/printing on the Python side
- **[Config & env](config.html)** -- `HYPERACTOR_FORCE_FILE_LOG`, `HYPERACTOR_PREFIX_WITH_RANK`, `MAX_LINE_SIZE`