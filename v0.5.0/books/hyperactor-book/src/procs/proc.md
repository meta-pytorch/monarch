...
- `proc_id`: The proc's unique identity (see [ProcId](../references/proc_id.html))
...
See [ProcId](../references/proc_id.html) for complete details.
...
See [Routers](../mailboxes/routers.html#dialmailboxrouter-remote-and-serializable-routing).

### Local Proc Bypass

When a proc sends a message to an actor in another proc, the message normally flows through the proc's forwarder to a backend address, where a central routing layer forwards it to the destination proc (see [Host](host.html) for the complete architecture with frontend and backend receivers). For procs running on the same host, this can be optimized: procs can dial each other directly via Unix sockets, bypassing the central routing layer entirely.
...
See [RemoteSpawn](../actors/remote_spawn.html) for spawning details.

## See Also

- [Host](host.html) - How hosts create and manage procs
- [ProcId](../references/proc_id.html) - Proc addressing schemes
- [Routers](../mailboxes/routers.html) - Forwarder implementations
- [ActorId](../references/actor_id.html) - How actors inherit proc identity