...
- `ActorRef<A>`: A typed reference to an actor implementing the `Referable` trait.
- `PortRef<M>`: A reference to a reusable mailbox port for messages of type `M` implementing the `RemoteMessage` trait.
- `OncePortRef<M>`: A reference to a one-shot port for receiving a single response of type `M` implementing the `RemoteMessage` trait.
