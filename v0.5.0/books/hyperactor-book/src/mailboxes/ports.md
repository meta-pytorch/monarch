...
Variants like `OncePortHandle` and `OncePortReceiver` support one-shot communication. All ports are associated with a unique `PortId` within an actor, and may be wrapped in a `PortRef<M>` for safe external use.
...
This registers the port in the owning `Mailbox` and returns a `PortRef<M>`. Binding is lazy and idempotent. For a detailed explanation of port binding, see [Mailbox](mailbox.html#port-binding).
...
Compared to `PortHandle<M>`, a `OncePortHandle<M>` is:
...
See the (Mailbox) [`State`](mailbox.html#state) section for details on how the mailbox owns and manages this ports map.
