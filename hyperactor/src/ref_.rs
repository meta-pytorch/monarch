...
impl ProcRef {
    ...
    ///
    /// Recognizes: `_label` (singleton), `label-hex16` (labeled instance),
    ...
    /// The ResourceId text form: `_label` (singleton), `label-hex16`
    /// (labeled instance), or `hex16` (unlabeled instance).
...
/// `hex16` (unlabeled instance). Falls back to singleton from stripped name.
pub fn parse_resource_name(s: &str) -> (Uid, Option<Label>) {
...
impl ActorRef {
    ...
    pub fn port_ref(&self, port: crate::port::Port) -> PortRef {
        PortRef::new(
