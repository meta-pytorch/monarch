...
//! feed a [`NetRx<M>`], duplex sessions feed a [`DuplexServer<In, Out>`].
    ...
    Out: RemoteMessage,
{
    ...
    // `ServerHandle::join` waits on this signal so `NetRx::join` and
    ...
    // (via [`NetRx::Drop`] / [`DuplexServer::Drop`]).
    ...
    let net_rx = NetRx::new(simplex_rx, channel_addr.clone(), simplex_handle);
    let duplex_server =
...
/// contract for [`NetRx`] and the equivalent contract for
...
mod tests {
    ...
    /// the shared shutdown via `NetRx`'s `Drop` impl, which calls
    ...
    /// Adversarial regression for the [`MuxServer::serve`] shutdown
    /// ordering bug. The duplex handler simulates an application that
    /// performs a deterministic post-stop send (mirroring the host's
    /// per-connection forwarder drain). The previous shutdown order
    /// fired the shared listener cancel before awaiting `duplex_task`,
    /// so the cancellation cascade through the per-session children
    /// would interrupt `dispatch_duplex_stream`'s `select!` mid-drain
    /// and the post-stop send would never reach the peer. With the
    /// fixed order, the duplex handler is awaited first and the send
    /// makes it through.
    #[async_timed_test(timeout_secs = 30)]
    async fn test_mux_stop_waits_for_duplex_cleanup_before_transport_cancel() {
        ...
        // Keep `rx` alive in the spawned pump task until either stop
        // fires or `rx` returns `Closed` (mirrors `MailboxServer::serve`'s
        // own loop: a Closed receive ends the pump). Without exiting
        // on Closed, the cancel cascade closing `simplex_rx` would
        // wake this pump in a busy loop and the coordinator's
        // `simplex_handle.await` would hang forever.
        ...
        let (cleanup_sent_tx, cleanup_sent_rx) = tokio::sync::oneshot::channel();

        let mut accepted_tx = Some(accepted_tx);
        let mut cleanup_sent_tx = Some(cleanup_sent_tx);
        let handle = frontend.serve(
            passthrough_mailbox_handle,
            move |mut duplex_server, stop_rx| async move {
                let (_server_rx, server_tx) = duplex_server.accept().await.unwrap();
                accepted_tx.take().unwrap().send(()).unwrap();

                wait_for_stop(stop_rx).await;

                // Sleep to widen the window where the bug would race
                // the listener cancel ahead of this delivery.
                tokio::time::sleep(Duration::from_millis(100)).await;
                server_tx.post(99);
                cleanup_sent_tx.take().unwrap().send(()).unwrap();

                // Mirror what the host's `duplex_accept_loop` does at the
                // very end of its drain — explicit listener teardown.
                duplex_server.join().await;
            },
        );

        accepted_rx.await.unwrap();
        handle.stop("test mux stop ordering");

        tokio::time::timeout(Duration::from_secs(5), cleanup_sent_rx)
            .await
            .expect("duplex cleanup task did not run")
            .unwrap();

        let got = tokio::time::timeout(Duration::from_secs(5), client_rx.recv())
            .await
            .expect("client did not receive duplex cleanup message")
            .expect("client rx closed before cleanup message");
        assert_eq!(got, 99);
