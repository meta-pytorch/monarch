...
impl ActorAttrsView {
    ...
    pub fn from_attrs(attrs: &Attrs) -> Result<Self, AttrsViewError> {
        ...
        let failure = if has_any_failure {
            ...
            Some(FailureAttrs {
                error_message,
                root_cause_actor: root_cause_actor.into(),
                root_cause_name,
                occurred_at,
                is_propagated,
            })
    ...
    pub fn to_attrs(&self) -> Attrs {
        ...
        if let Some(fi) = &self.failure {
            ...
            attrs.set(
                FAILURE_ROOT_CAUSE_ACTOR,
                reference::ActorId::from(fi.root_cause_actor.clone()),
            );
    ...
    let failure = if status.is_failed() {
        cell.supervision_event().and_then(|event| {
            ...
            Some(FailureSnapshot {
                ...
                is_propagated: root.actor_id != ref_::ActorRef::from(actor_id.clone()),
            })
        })
    } else {
        None
...
mod tests {
    ...
    fn test_fi7_fi8_propagated_stopped_child() {
        ...
        attrs.set(
            FAILURE_ROOT_CAUSE_ACTOR,
            crate::reference::ActorId::from(snap.root_cause_actor.clone()),
        );
