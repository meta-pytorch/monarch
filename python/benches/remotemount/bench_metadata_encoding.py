...
def make_synthetic_metadata(num_files: int, num_dirs: int = 0) -> dict:
...
def bench_json_encode(meta: dict, iterations: int = 100) -> tuple[float, int]:
...
def bench_mount_metadata(
    meta: dict, chunks: list, chunk_size: int, iterations: int = 20
) -> float:
