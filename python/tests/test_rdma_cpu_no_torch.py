...
class CpuActor(Actor):
    ...
    async def create_buffer(self) -> RDMABuffer:
        ...
        print(f"[ACTOR TIMING] RDMABuffer() total: {(t1-t0)*1000:.1f}ms", flush=True)
        return self.buf
    ...
    async def read_buffer(self, buf: RDMABuffer) -> bytes:
        ...
        t1 = _time.perf_counter()
        print(f"[ACTOR TIMING] read_buffer: buf.read_into(): {(t1-t0)*1000:.1f}ms", flush=True)
        return bytes(dst)
...
async def test_rdma_buffer_cpu_memoryview(rdma_backend):
    ...
    with cm:
        ...
        print(f"[TIMING] create_buffer (sender, after ping): {(t3-t_ping)*1000:.1f}ms")

        received = await receiver.read_buffer.call_one(buf)
        t4 = time.perf_counter()
        print(f"[TIMING] read_buffer (receiver, includes QP setup + transfer): {(t4-t3)*1000:.1f}ms")
