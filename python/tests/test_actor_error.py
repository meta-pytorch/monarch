...
async def test_supervision_with_mesh_namespace(mesh, error_actor_cls) -> None:
    """Test that actor mesh refs loaded from namespace observe failures through monitoring.

    This test verifies that when an actor mesh is loaded from the namespace
    and the original actor experiences a failure, the loaded ref
    also observes the failure (since both refs point to the same underlying mesh).
    """
    ...
    # The loaded ref from namespace should also observe the failure through monitoring
    with pytest.raises(
        RuntimeError,
        match="Actor .* (is unhealthy with reason|exited because of the following reason)",
    ):
