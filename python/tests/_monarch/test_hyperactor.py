...
def test_buffer_write_rejects_non_contiguous_buffer() -> None:
    data = np.arange(16, dtype=np.uint8).reshape(4, 4).T
    assert not data.flags.c_contiguous

    with pytest.raises(BufferError, match="buffer is not C-contiguous"):
        Buffer().write(data)
