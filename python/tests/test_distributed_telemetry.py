...
def test_store_pyspy_dump_with_child_proc_ref(cleanup_callbacks) -> None:
    ...
    # ProcAgent is a system actor, so use its canonical ActorId suffix rather than
    # display_name, which is reserved for user-facing actor names.
    proc_agents = engine.query(
        "SELECT full_name FROM actors WHERE full_name LIKE '%,_proc_agent'"
    )
    proc_agent_names = proc_agents.to_pydict().get("full_name", [])
    child_proc_refs = [
        row.rsplit(",", 1)[0]
        for row in proc_agent_names
        if "," in row and row.rsplit(",", 1)[0] != coordinator_proc_id
    ]
