...
async def test_proc_mesh_churn_no_leaked_undeliverables():
    ...
    # Overlapping spawn/stop is what exposes the race, but too many
    # concurrent subprocess forks overwhelm the underlying proc-mesh
    # spawn and cause its 30s spawn timeout to fire. 4 x 2 GPUs = 8
    # concurrent proc spawns is comfortable and still produces plenty
    # of interleaving between casts and stops.
    max_in_flight = 4
    try:
