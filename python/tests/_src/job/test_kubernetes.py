...
class TestAddMesh(unittest.TestCase):
    ...
    def test_resources_requires_image(self) -> None:
        job = self._make_job()
        with self.assertRaises(ValueError, msg="requires 'image'"):
            job.add_mesh(
                "workers",
                num_replicas=1,
                resources={"cpu": "2"},
    ...
    def test_label_selector_forbidden_with_provisioning(self) -> None:
        job = self._make_job()
        with self.assertRaises(ValueError, msg="label_selector"):
            job.add_mesh(
                "workers",
                num_replicas=1,
                image="img",
    ...
    def test_pod_rank_label_forbidden_with_provisioning(self) -> None:
        job = self._make_job()
        with self.assertRaises(ValueError, msg="pod_rank_label"):
            job.add_mesh(
                "workers",
                num_replicas=1,
                image="img",
...
class TestBuildPodSpec(unittest.TestCase):
    """Tests for KubernetesJob._build_pod_spec."""

    def test_basic_pod_spec(self) -> None:
        spec = KubernetesJob._build_pod_spec("myimage:latest", port=26600)
