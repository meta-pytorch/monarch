...
def flatten(
    obj: Any, filter: Callable[[Any], bool]
) -> Tuple[List[Any], bytes | FrozenBuffer]:
    buffer = Buffer()
    pickler = _Pickler(filter, buffer)
    pickler.dump(obj)

    if isinstance(pickler.f, io.BytesIO):
        return pickler._saved, pickler.f.getvalue()

    return pickler._saved, buffer.freeze()


def unflatten(data: bytes | FrozenBuffer, values: Iterable[Any]) -> Any:
