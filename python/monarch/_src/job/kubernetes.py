...
class KubernetesJob(JobTrait):
    def __init__(
        ...
        Assumes each pod contains a single container running a bootstrapped Monarch Host.
        ...
        # mesh_name -> {"label_selector": str, "num_replicas": int, "pod_rank_label": str}
    ...
    def add_mesh(
        ...
        """
        Add a mesh specification. Meshes are discovered by label selector which expects to contain app=monarch-worker and monarch-mesh=<name> by default if using MonarchMesh CRD and operator.
        ...
        Args:
            ...
            label_selector: Custom Kubernetes label selector for pod discovery (default: "app=monarch-worker,monarch-mesh=<name>")
            pod_rank_label: Label key containing the pod rank for ordering workers (default: "apps.kubernetes.io/pod-index")
        """
        self._meshes[name] = {
            "label_selector": label_selector
            or f"app=monarch-worker,monarch-mesh={name}",
    ...
    def _is_pod_worker_ready(self, pod: client.V1Pod) -> bool:
        ...
        Assumes the pod contains a single container running a monarch worker.

        Args:
            pod: Kubernetes pod object

        Returns:
            True if pod is running and container is ready

        Raises:
            AssertionError: If pod contains more than one container
        """
        if pod.status.phase != "Running" or not pod.status.pod_ip:
            return False

        # Verify single container assumption
        if pod.status.container_statuses:
            assert len(pod.status.container_statuses) == 1, (
                f"Pod {pod.metadata.name} has {len(pod.status.container_statuses)} containers, "
        ...
        try:
            for event in w.stream(
                ...
                if timeout and (time.time() - start_time) >= timeout:
                    missing_ranks = set(range(num_replicas)) - set(
                        ready_pods_by_rank.keys()
                    )
                    raise RuntimeError(
                        f"Timeout after {timeout}s. Found {len(ready_pods_by_rank)}/{num_replicas} ranks. "
    ...
    def _discover_monarch_port(self, pod: client.V1Pod) -> int:
        ...
        Assumes the pod contains a single container. Checks in order:
        ...
        """
        # Already verified single container assumption
