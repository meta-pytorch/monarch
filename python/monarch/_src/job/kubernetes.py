...
class KubernetesJob(JobTrait):
    ...
    *Attach-only* -- connect to pre-provisioned pods discovered via label
    selectors. Compatible with the MonarchMesh operator, third-party
    schedulers, or manually created pods.
    ...
    *Provisioning* -- create MonarchMesh CRDs via the K8s API so the
    ...
    to ``add_mesh`` to enable provisioning for that mesh.
    """
    ...
    def add_mesh(
        ...
        if image is not None:
            mesh_entry["pod_spec"] = self._build_pod_spec(image, port, resources)
