...
pub fn certificate_pid(certificate: &[u8]) -> Pid {
    ...
    pid_from_digest(&digest)
}

fn pid_from_digest(digest: &[u8]) -> Pid {
    let mut pid = [0; PID_LEN];
    pid.copy_from_slice(&digest[..PID_LEN]);
    if pid == *Pid::LINK_LOCAL.as_bytes() {
        pid[PID_LEN - 1] = 1;
    }
    Pid::from_bytes(pid)
}
...
mod tests {
    ...
    #[test]
    fn reserved_pid_is_remapped() {
        assert!(!pid_from_digest(&[0; 32]).is_link_local());
    }
}
