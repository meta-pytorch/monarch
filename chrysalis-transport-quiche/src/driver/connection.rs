...
use super::network::InitialRoute;
use super::*;
...
pub(super) struct ConnectionState {
    ...
    local_route: Pid,
    pub(super) route: [u8; CID_LEN],
    pub(super) initial_route: Option<InitialRoute>,
...
impl ConnectionState {
    ...
    pub(super) fn authentication_pending(&self) -> bool {
        matches!(self.authentication, AuthenticationState::Pending { .. })
    }

    pub(super) fn peer(&self) -> Option<Pid> {
        match self.authentication {
            AuthenticationState::Authenticated(peer) => Some(peer),
            AuthenticationState::Pending { .. } | AuthenticationState::Failed => None,
        }
    }
    ...
    fn enqueue_receive(stream: &mut StreamState, submission: ReceiveSubmission) {
        let (operation, _, buffer, options, cancellation) = submission.into_parts();
    ...
    pub(super) fn progress(
        ...
        if self.authentication == AuthenticationState::Failed {
            return false;
        }
            ...
            && let AuthenticationState::Pending { expected } = self.authentication
        {
            ...
            let peer = actual.expect("authenticated peer certificate should produce a PID");
            self.authentication = AuthenticationState::Authenticated(peer);
            completions.push(Completion::ConnectionEstablished(
                ConnectionEstablishedCompletion::new(connection_id, self.local_route, peer),
            ));
        }
        ...
        while let Some(stream_id) = self.runnable_streams.pop_front() {
            ...
            self.reap_terminal_stream(connection_id, stream_id, completions);
        }
        became_established
    }
    ...
    pub(super) fn reap_terminal_stream(
        &mut self,
        connection_id: ConnectionId,
        stream_id: u64,
        completions: &mut Vec<Completion>,
    ...
    fn progress_receive_ops(
        ...
        while !stream.receive_ops.is_empty() {
                ...
                match stream.receive_ops.front_mut().unwrap() {
                    ReceiveOp::Receive(pending) => {
                        if pending.cancellation.is_cancelled() {
                            Some(if pending.buffer.buffer().len() == pending.initial_length {
                            ...
                            let result = {
                                let initial_length = pending.buffer.buffer().len();
                                match connection.stream_recv(stream_id, pending.buffer.writable()) {
                                    Ok((read, fin)) => {
                                        pending.buffer.truncate(initial_length + read);
                                        Ok((read, fin))
                                    }
                                    Err(error) => {
                                        pending.buffer.truncate(initial_length);
                                        Err(error)
                                    }
                                }
                            };
            ...
            match stream.receive_ops.pop_front().unwrap() {
                ReceiveOp::Receive(pending) => {
                    let _ = pending.cancellation.complete();
            ...
            while let Some(operation) = stream.receive_ops.pop_front() {
                match operation {
                    ReceiveOp::Receive(receive) => {
                        let status = if receive.cancellation.complete() {
