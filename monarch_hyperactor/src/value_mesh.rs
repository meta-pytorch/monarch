...
pub struct PyValueMesh {
    /// The shape of the mesh (labels + ndslice)
    shape: Shape,
...
impl PyValueMesh {
    ...
    fn item(&self, py: Python<'_>, kwargs: Option<&Bound<'_, PyDict>>) -> PyResult<PyObject> {
        ...
        let ranks: Vec<usize> = self.shape.slice().iter().collect();
        let local_idx = ranks
            .iter()
    ...
    fn items(&self, py: Python<'_>) -> PyResult<PyObject> {
        let extent = self.shape.extent();
        let mut result: Vec<(PyPoint, Py<PyAny>)> = Vec::with_capacity(self.shape.slice().len());

        for (i, _global_rank) in self.shape.slice().iter().enumerate() {
            let point = PyPoint::new(i, PyExtent::from(extent.clone()));
            let value = Ranked::get(&self.inner, i).unwrap().clone_ref(py);
    ...
    /// - Shape is serialized with bincode (Rust-native, fast, compact)
    ...
    fn __reduce__<'py>(
        ...
        let shape_bytes =
            bincode::serialize(&this.shape).map_err(|e| PyValueError::new_err(e.to_string()))?;
        ...
        let combined = (shape_bytes, values_bytes.extract::<Vec<u8>>()?);
        let combined_bytes =
    ...
    fn __repr__(&self, py: Python<'_>) -> PyResult<String> {
        ...
        for (i, _global_rank) in self.shape.slice().iter().enumerate() {
            let point = extent
                .point_of_rank(i)
                .map_err(|e| PyValueError::new_err(e.to_string()))?;
            let value = Ranked::get(&self.inner, i).unwrap();

            let coords = point.coords();
            let labels = extent.labels();
            let sizes = extent.sizes();
            let mut point_parts = Vec::new();
            for (j, label) in labels.iter().enumerate() {
                point_parts.push(format!("'{}': {}/{}", label, coords[j], sizes[j]));
            }
            let point_str = format!("{{{}}}", point_parts.join(", "));

            let value_repr: String = value.bind(py).repr()?.extract()?;

            items_str.push_str(&format!("  ({}, {}),\n", point_str, value_repr));
        }
...
impl PyValueMesh {
    /// Remap values to a new shape (internal implementation).
    ///
    /// This is the core logic for `new_with_shape` - it maps global ranks
    /// from the new shape back to local indices in the current shape.
    fn remap_to_shape(&self, py: Python<'_>, new_shape: Shape) -> PyResult<Self> {
        let cur_ranks: Vec<usize> = self.shape.slice().iter().collect();
        let pos: HashMap<usize, usize> =
            cur_ranks.iter().enumerate().map(|(i, &g)| (g, i)).collect();

        let mut remapped: Vec<Py<PyAny>> = Vec::with_capacity(new_shape.slice().len());
        for global_rank in new_shape.slice().iter() {
            let local_idx = pos.get(&global_rank).ok_or_else(|| {
                PyValueError::new_err(format!("rank {} not in current shape", global_rank))
            })?;
            let value = Ranked::get(&self.inner, *local_idx).unwrap().clone_ref(py);
            remapped.push(value);
        }

        let region = Region::new(new_shape.labels().to_vec(), new_shape.slice().clone());
        let mut inner =
            <ValueMesh<Py<PyAny>> as BuildFromRegion<Py<PyAny>>>::build_dense(region, remapped)
                .map_err(|e| PyValueError::new_err(e.to_string()))?;
        ...
        Python::with_gil(|py| {
            ...
            match result {
                Err(e) => {
                    ...
                    assert!(
                        err_msg.contains("not in current shape"),
                        "Expected 'not in current shape' error, got: {}",
                        err_msg
                    );
                }
