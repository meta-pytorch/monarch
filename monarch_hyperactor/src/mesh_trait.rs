...
//!     fn mesh_shape(&self) -> &Shape { &self.shape }
...
pub trait PyMeshTrait: Sized {
    /// Returns a reference to the mesh's shape (labels + NDSlice).
    ...
    fn mesh_slice(&self, py: Python<'_>, kwargs: Option<&Bound<'_, PyDict>>) -> PyResult<Self> {
        let kwargs = match kwargs {
            ...
            _ => return self.new_with_shape(py, self.mesh_shape().clone()),
        };
        self.new_with_shape(py, shape_slice(self.mesh_shape().clone(), kwargs)?)
    }
...
mod tests {
    ...
    // A simple test struct implementing PyMeshTrait
    #[derive(Debug)]
    struct TestMesh {
        shape: Shape,
    }
    ...
    fn test_slice_with_range() {
        // Slice object keeps dimension but restricts range
        pyo3::prepare_freethreaded_python();
        Python::with_gil(|py| {
            let mesh = TestMesh {
                shape: shape!(host = 2, gpu = 8),
