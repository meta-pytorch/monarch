...
fn encode(message: &Message) -> Result<Vec<u8>, ProtocolError> {
    ...
    match message {
        ...
        Message::Schema(schema) => {
            encoder.byte(SCHEMA);
            encoder.string(&schema.table)?;
            encoder.string(&schema.create_sql)?;
            encoder.bytes(&schema.hash)?;
        }
...
fn decode(bytes: &[u8]) -> Result<Message, ProtocolError> {
    ...
    let message = match decoder.byte()? {
        ...
        SCHEMA => {
            ...
            let create_sql = decoder.string()?;
            let hash = decoder
                .bytes()?
                .try_into()
                .map_err(|_| ProtocolError::InvalidSchema)?;
            Message::Schema(
                TableSchema::from_wire(table, create_sql, hash)
                    .map_err(|_| ProtocolError::InvalidSchema)?,
            )
        }
...
impl Encoder {
    ...
    fn change(&mut self, change: &Change) -> Result<(), ProtocolError> {
        ...
        self.bytes(&change.pk)?;
        self.string(&change.cid)?;
        self.value(&change.value)?;
        self.i64(change.col_version);
...
mod tests {
    ...
    fn messages() -> Vec<Message> {
        vec![
            ...
            Message::Schema(TableSchema::new(
                "items".into(),
                "CREATE TABLE items (id INTEGER PRIMARY KEY NOT NULL)".into(),
            )),
    ...
    async fn cancelled_receive_resumes_partial_frame() {
        let expected = Message::Schema(TableSchema::new(
            "items".into(),
            "CREATE TABLE items (id INTEGER PRIMARY KEY NOT NULL)".into(),
        ));
    ...
    fn rejects_trailing_and_unknown_data() {
        ...
        let mut encoded = encode(&Message::Schema(TableSchema::new(
            "items".into(),
            "CREATE TABLE items (id INTEGER PRIMARY KEY NOT NULL)".into(),
        )))
        .unwrap();
