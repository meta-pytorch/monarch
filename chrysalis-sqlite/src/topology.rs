...
/// Composes one CRR replica with Node-managed parent and child links.
#[derive(Clone)]
...
impl ReplicationTopology {
    ...
    /// Registers CRR replication on every link managed by the node.
    ...
    async fn run(&self, context: LinkContext, stream: Stream) {
        match context.side() {
            LinkSide::Parent => {
                ...
                {
                    eprintln!(
                        "SQLite replication with {:?} failed: {error}",
                        context.peer()
                    );
                }
            }
...
impl ChildGuard {
    fn enter(inner: Arc<Inner>, context: LinkContext) -> Self {
        ...
        assert!(replaced.is_none(), "child link already has a CRR session");
        Self { inner, context }
    }
}
