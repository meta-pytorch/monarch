...
`clear_runtime_config()` in long-running processes.
...
monarch.config.configure(***kwargs*)[[source]](../_modules/monarch/config.html#configure)
...
- **behavior** (*Basic logging*) - enable_log_forwarding: Forward child stdout/stderr through the mesh.
enable_file_capture: Persist child stdout/stderr to per-host files.
tail_log_lines: Number of log lines to retain in memory.
- **delivery** (*Message encoding and*) - codec_max_frame_length: Maximum serialized message size in bytes.
...
- **configuration** - read_log_buffer: Buffer size for reading logs (bytes).
...
- **timeouts** - proc_stop_max_idle: Maximum idle time while stopping procs (humantime).
...
****overrides** (*ConfigureKwargsType*) - Configuration key-value pairs to override for the
...
monarch.config.get_global_config()[[source]](../_modules/monarch/config.html#get_global_config)
...
*not* change the active configuration; use `configure()` instead.

monarch.config.get_runtime_config()[[source]](../_modules/monarch/config.html#get_runtime_config)

Return a snapshot of just the Runtime layer configuration.

Useful for snapshot/restore flows (see `configured()`) or for
inspecting which keys were last set via Python.

monarch.config.clear_runtime_config()[[source]](../_modules/monarch/config.html#clear_runtime_config)
...
typically paired with `configure()` to reset overrides in long-lived
processes.
...
Uses [humantime](https://docs.rs/humantime/latest/humantime/) format.
...
[humantime](https://docs.rs/humantime/latest/humantime/) syntax;
...
- [Getting Started](../generated/examples/getting_started.html) - Getting started guide
- [monarch.actor](monarch.actor.html) - Actor API documentation