...
Remote-spawnable actors are registered using the `remote!` macro. Given an actor type that implements [`RemoteSpawn`](remote_spawn.html) and `Named`, `remote!(MyActor)` arranges for a `SpawnableActor` record to be submitted to a global registry using the [`inventory`](https://docs.rs/inventory/0.3.21/inventory/index.html) crate.
