...
- [Client actor](client.html) -- aggregation, similarity bucketing, barriers
- [Forwarder internals](forwarder.html) -- in-band `LogMessage::{Log, Flush}` and heartbeats
- [Stream forwarders](stream-forwarders.html) -- FD capture, `tee`, local files
- [Python control surface](python.html) -- `spawn`, `set_mode`, `flush`