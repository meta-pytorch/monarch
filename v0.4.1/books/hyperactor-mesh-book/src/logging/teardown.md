...
- **[Client actor](client.html)** -- aggregation & barriers (source excerpts included)
- **[Forwarder internals](forwarder.html)** -- why `Flush(None)` exists
- **[Stream forwarders](stream-forwarders.html)** -- where EOF is handled and the final `flush()` is posted
- **Teardown** -- what teardown does/doesn't guarantee
- **[Config & env](config.html)** -- knobs affecting teardown behavior