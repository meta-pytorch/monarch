...
use chrysalis_sqlite::load_crsqlite;
use clap::Parser;
use clap::Subcommand;
use clap::ValueEnum;
use libsql::Builder;
...
impl FromStr for LabelArg {
    ...
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        let (key, value) = value
            .split_once('=')
            .ok_or_else(|| "label must have the form KEY=VALUE".to_owned())?;
        Ok(Self {
            key: key.parse().map_err(|error| format!("{error}"))?,
            value: value.parse().map_err(|error| format!("{error}"))?,
        })
    }
}
...
enum Command {
    ...
    /// Replicate and inspect a local cr-sqlite database
    Sqlite {
        #[command(subcommand)]
...
#[derive(Debug, Subcommand)]
enum SqliteCommand {
    /// Join the process mesh and continuously synchronize a database file
    Sync { database: PathBuf },
    /// Synchronize a database and open an interactive SQLite shell
    Repl {
...
async fn main() -> Result<()> {
    ...
    let cluster = match (cluster, join) {
        ...
        (None, Some(cluster)) => {
            eprintln!("warning: --join is deprecated; use --cluster");
            Some(cluster)
        }
        (None, None) => None,
    ...
    let identity = match identity_kind {
        IdentityKind::Ephemeral => identity::generate()?,
    ...
    let sqlite_database = match &command {
        Command::Sqlite {
            command: Some(SqliteCommand::Sync { database } | SqliteCommand::Repl { database }),
        } => Some(database),
        _ => None,
    };
    let mut sqlite = None;
    if let Some(database) = sqlite_database {
        ...
        let replica = Replica::new(connection.clone()).await?;
        sqlite = Some(SqliteRuntime {
            connection,
            replica: replica.clone(),
        });
        config = ReplicationTopology::new(replica).configure(config);
    }
    let node = Arc::new(Node::create(config)?);
    let parent = wait_until_ready(&node).await?;
    let result = match command {
        ...
        Command::Sqlite {
            command: Some(SqliteCommand::Sync { database }),
        } => {
            sqlite_sync(
                &node,
                address,
                ...
                sqlite
                    .as_ref()
                    .expect("sync command initialized a SQLite replica")
                    .replica
                    .site_id()
                    .to_vec(),
            )
        ...
        Command::Sqlite {
            ...
            let sqlite = sqlite
                .as_ref()
                .expect("repl command initialized a SQLite replica");
            sqlite_repl(&node, address, &database, sqlite, parent).await
        }
        Command::Sqlite {
            command: Some(SqliteCommand::Query { .. }),
        } => unreachable!("query command returned before node construction"),
        Command::Sqlite { command: None } => {
            unreachable!("missing SQLite command was normalized to repl")
        }
    };
    node.shutdown();
    node.join().await;
...
async fn resolve_connection(
    ...
    let carrier = match carrier {
        ...
        None => resolved
            .as_ref()
            .map(|resolved| resolved.carrier().parse())
            .transpose()
...
async fn open_sqlite(path: &Path) -> Result<Connection> {
    ...
    let extension = packaged_extension_path()?;
    load_crsqlite(&connection, &extension)
        .with_context(|| format!("load cr-sqlite into {path}"))?;
    Ok(connection)
}

async fn sqlite_sync(
    node: &Node,
    address: NodeAddr,
    database: &Path,
    site_id: Vec<u8>,
    ...
    eprintln!(
        "synchronizing {} as site {} on {}",
        database.display(),
        format_bytes(&site_id),
        format_pid(node.pid()),
    );
    tokio::signal::ctrl_c()
        .await
        .context("wait for interrupt while synchronizing SQLite")?;
    eprintln!("stopping SQLite synchronization for {}", database.display());
    Ok(())
}
...
async fn sqlite_repl(
    ...
    eprintln!(
        "synchronizing {} as site {} on {}",
        database.display(),
        format_bytes(sqlite.replica.site_id()),
        format_pid(node.pid()),
    );
    eprintln!("connect: {address}");
    if let Some(parent) = parent {
        wait_for_sqlite_synchronization(&sqlite.replica, parent).await?;
    }
    sqlite_shell::run(&sqlite.connection).await
}

async fn wait_for_sqlite_synchronization(replica: &Replica, parent: Pid) -> Result<()> {
    const FRAMES: &[char] = &['⠋', '⠙', '⠹', '⠸', '⠼', '⠴', '⠦', '⠧', '⠇', '⠏'];

    let mut changes = replica.subscribe_peer_synchronization();
    let mut interval = tokio::time::interval(Duration::from_millis(80));
    interval.set_missed_tick_behavior(tokio::time::MissedTickBehavior::Delay);
    let animated = std::io::stderr().is_terminal();
    let mut frame = 0;
    if !animated {
        eprintln!("synchronizing SQLite with {}", format_pid(parent));
    }
    while !replica.is_peer_synchronized(parent) {
        tokio::select! {
            changed = changes.changed() => {
                changed.context("SQLite synchronization state closed")?;
            }
            _ = interval.tick(), if animated => {
                let status = if replica.peer_scopes().contains_key(&parent) {
                    "synchronizing SQLite with"
                } else {
                    "starting SQLite replication with"
                };
                eprint!(
                    "\r{} {status} {}",
                    FRAMES[frame],
                ...
                std::io::stderr()
                    .flush()
                    .context("flush SQLite synchronization progress")?;
                frame = (frame + 1) % FRAMES.len();
            }
            result = tokio::signal::ctrl_c() => {
                result.context("wait for interrupt while synchronizing SQLite")?;
                if animated {
                    eprint!("\r\x1b[2K");
                }
                anyhow::bail!("interrupted while synchronizing SQLite");
            }
        }
    }
    if animated {
    ...
    eprintln!("synchronized SQLite with {}", format_pid(parent));
    Ok(())
}
...
#[cfg(fbcode_build)]
fn packaged_extension_path() -> Result<PathBuf> {
    Ok(buck_resources::get("monarch/chrysalis-cli/crsqlite")?.to_path_buf())
}

#[cfg(not(fbcode_build))]
fn packaged_extension_path() -> Result<PathBuf> {
    anyhow::bail!("the OSS CLI does not yet package cr-sqlite")
}

async fn wait_until_ready(node: &Node) -> Result<Option<Pid>> {
    let Some(mut parent) = node.subscribe_parent() else {
        eprintln!("started root {}", format_pid(node.pid()));
        return Ok(None);
    };
    loop {
...
async fn serve(node: Arc<Node>, address: NodeAddr) -> Result<()> {
    println!("{address}");
    std::io::stdout().flush().context("flush join token")?;
    eprintln!("serving {} on {}", format_pid(node.pid()), address.address);
    let mut streams = JoinSet::new();
    loop {
        tokio::select! {
            incoming = node.accept() => {
                let incoming = incoming.context("accept application stream")?;
                eprintln!("accepted stream from {}", format_pid(incoming.source()));
                streams.spawn(handle_stream(incoming));
            }
            completed = streams.join_next(), if !streams.is_empty() => {
                completed.expect("stream set is not empty")??;
            }
            result = tokio::signal::ctrl_c() => {
                result.context("wait for interrupt")?;
                eprintln!("shutting down {}", format_pid(node.pid()));
                node.shutdown();
                break;
            }
        }
...
async fn handle_stream(incoming: IncomingStream) -> Result<()> {
    ...
    loop {
        let length = AsyncReadExt::read(&mut recv, &mut buffer)
            .await
            .with_context(|| format!("read stream from {}", format_pid(source)))?;
        if length == 0 {
            break;
        }
        stdout.write_all(&buffer[..length]).await?;
        stdout.flush().await?;
        send.write_all(&buffer[..length]).await?;
    }
    send.finish().await?;
    eprintln!("closed stream from {}", format_pid(source));
    Ok(())
}
...
async fn cat(node: &Node, target: Reference) -> Result<()> {
    ...
    eprintln!("connected to {}", format_pid(target));
    let (mut send, mut recv) = stream.into_parts();
...
mod tests {
    ...
    use chrysalis_sqlite::table_schemas;
    use tokio::io::duplex;
    ...
    #[tokio::test]
    async fn synchronization_marker_follows_schema_and_rows() {
        const PARENT: Pid = Pid::from_bytes([1; 16]);
        const CHILD: Pid = Pid::from_bytes([2; 16]);

        let parent_connection = open_sqlite(Path::new(":memory:"))
            .await
            .expect("open parent database");
        parent_connection
            .execute_batch(
                "CREATE TABLE alpha (\
                 ...
                 SELECT crsql_as_crr('alpha'); \
                 INSERT INTO alpha VALUES (1, 'first'); \
                 CREATE TABLE beta (id INTEGER PRIMARY KEY NOT NULL); \
                 SELECT crsql_as_crr('beta');",
            )
            .await
            .expect("initialize parent database");
        let child_connection = open_sqlite(Path::new(":memory:"))
            .await
            .expect("open child database");
        let parent = Replica::new(parent_connection)
            .await
            .expect("create parent replica");
        let child = Replica::new(child_connection.clone())
            .await
            .expect("create child replica");
        let (parent_io, child_io) = duplex(64 * 1024);
        let (parent_reader, parent_writer) = tokio::io::split(parent_io);
        ...
        let child_task = tokio::spawn({
            ...
            async move {
                child
                    .replicate_io(PARENT, child_writer, child_reader, child.local_publisher())
                    .await
            }
        });

        let mut synchronization = child.subscribe_peer_synchronization();
        timeout(Duration::from_secs(5), async {
            while !child.is_peer_synchronized(PARENT) {
                synchronization
                    .changed()
                    .await
                    .expect("synchronization sender remains active");
            }
        })
        .await
        .expect("child synchronizes with parent");

        let schemas = table_schemas(&child_connection)
            .await
            .expect("read child schemas");
        assert_eq!(schemas.keys().collect::<Vec<_>>(), ["alpha", "beta"]);
        let mut rows = child_connection
            .query("SELECT value FROM alpha WHERE id = 1", ())
            .await
            .expect("query replicated row");
        assert_eq!(
            rows.next()
                .await
                .expect("read replicated row")
                .expect("replicated row exists")
                .get::<String>(0)
