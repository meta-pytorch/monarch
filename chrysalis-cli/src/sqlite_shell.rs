...
pub(crate) async fn run(connection: &Connection) -> Result<()> {
    ...
    let result = loop {
        ...
        if sql.is_empty() && line.trim_start().starts_with('.') {
            record_history(&mut editor, line.trim());
            if run_meta_command(connection, line.trim()).await? {
                break Ok(());
            }
            continue;
        }
        ...
        record_history(&mut editor, sql.trim_end());
...
fn record_history(editor: &mut DefaultEditor, entry: &str) {
    if let Err(error) = editor.add_history_entry(entry) {
        eprintln!("Warning: could not add SQLite history entry: {error}");
    }
}
...
async fn run_meta_command(connection: &Connection, command: &str) -> Result<bool> {
    ...
    match name {
        ...
        ".help" => {
            println!(".tables              List application tables and views");
            println!(".schema [TABLE]      Show application schema");
            ...
            print_meta_query(
                ...
                "SELECT name FROM sqlite_schema \
                 WHERE type IN ('table', 'view') \
                   ...
                   AND name NOT LIKE '__chrysalis_crr_%' \
                 ORDER BY name",
            )
            .await?;
        }
        ".schema" => {
            let filter = if argument.is_empty() {
                String::new()
            } else {
                format!(" AND tbl_name = '{}'", argument.replace('\'', "''"))
            };
            print_meta_query(
                connection,
                &format!(
...
async fn print_meta_query(connection: &Connection, sql: &str) -> Result<()> {
    let output = execute(connection, sql).await?;
    print!("{output}");
    std::io::stdout()
        .flush()
