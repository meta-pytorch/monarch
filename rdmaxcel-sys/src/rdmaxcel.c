...
struct ibv_qp* create_efa_qp(struct ibv_context* context, struct ibv_pd* pd,
    struct ibv_cq* send_cq, struct ibv_cq* recv_cq,
    int max_send_wr, int max_recv_wr, int max_send_sge, int max_recv_sge) {
...
int rdmaxcel_efa_post_write(rdmaxcel_qp_t* qp, struct ibv_ah* ah,
    uint32_t remote_qpn, uint32_t qkey,
    void* local_addr, uint32_t lkey, size_t length,
    void* remote_addr, uint32_t rkey, uint64_t wr_id) {
  if (!qp || !qp->qpex || !ah) {
    return RDMAXCEL_INVALID_PARAMS;
  }
  ...
  if (ret != 0) {
    fprintf(stderr, "ERROR: EFA ibv_wr_complete (write) failed: %d (%s)\n", ret, strerror(ret));
    return RDMAXCEL_WR_COMPLETE_FAILED;
  }
  return RDMAXCEL_SUCCESS;
}

int rdmaxcel_efa_post_read(rdmaxcel_qp_t* qp, struct ibv_ah* ah,
    uint32_t remote_qpn, uint32_t qkey,
    void* local_addr, uint32_t lkey, size_t length,
    void* remote_addr, uint32_t rkey, uint64_t wr_id) {
  if (!qp || !qp->qpex || !ah) {
  ...
  int ret = ibv_wr_complete(qp->qpex);
  if (ret != 0) {
    fprintf(stderr, "ERROR: EFA ibv_wr_complete (read) failed: %d (%s)\n", ret, strerror(ret));
    return RDMAXCEL_WR_COMPLETE_FAILED;
  }
  return RDMAXCEL_SUCCESS;
}
...
int rdmaxcel_efa_connect(rdmaxcel_qp_t* qp,
    uint8_t port_num, uint16_t pkey_index, uint32_t qkey,
    uint32_t psn, uint8_t gid_index,
  ...
  if (ret != 0) {
    fprintf(stderr, "ERROR: EFA failed to transition QP to INIT: %d (%s)\n",
        ret, strerror(ret));
  ...
  if (ret != 0) {
    fprintf(stderr, "ERROR: EFA failed to transition QP to RTR: %d (%s)\n",
        ret, strerror(ret));
    return RDMAXCEL_QP_MODIFY_FAILED;
  }
  ...
  if (ret != 0) {
    fprintf(stderr, "ERROR: EFA failed to transition QP to RTS: %d (%s)\n",
        ret, strerror(ret));
    return RDMAXCEL_QP_MODIFY_FAILED;
  }
  ...
  if (!ah) {
    fprintf(stderr, "ERROR: EFA failed to create address handle: %s\n",
        strerror(errno));
  ...
  uint64_t rts_nanos = (uint64_t)ts.tv_sec * 1000000000ULL + (uint64_t)ts.tv_nsec;
...
int rdmaxcel_efa_post_op(rdmaxcel_qp_t* qp, struct ibv_ah* ah,
    uint32_t remote_qpn, uint32_t qkey,
    void* local_addr, uint32_t lkey, size_t length,
    void* remote_addr, uint32_t rkey, uint64_t wr_id,
    int op_type) {
  ...
  if (op_type == 0) {
    return rdmaxcel_efa_post_write(qp, ah, remote_qpn, qkey,
        local_addr, lkey, length, remote_addr, rkey, wr_id);
  } else if (op_type == 1) {
    return rdmaxcel_efa_post_read(qp, ah, remote_qpn, qkey,
        local_addr, lkey, length, remote_addr, rkey, wr_id);
  }
...
int rdmaxcel_qp_post_op(rdmaxcel_qp_t* qp,
    void* local_addr, uint32_t lkey, size_t length,
    void* remote_addr, uint32_t rkey, uint64_t wr_id,
    int signaled, int op_type) {
  if (!qp || !qp->ibv_qp) {
    return RDMAXCEL_INVALID_PARAMS;
  }
  ...
  if (op_type == 2) {
    ...
    if (ret != 0) {
      fprintf(stderr, "ERROR: ibv_post_recv failed: %d (%s)\n", ret, strerror(ret));
      return RDMAXCEL_WR_COMPLETE_FAILED;
    }
    return RDMAXCEL_SUCCESS;
  }
  ...
  return rdmaxcel_efa_post_op(qp, qp->efa_ah, qp->efa_remote_qpn, qp->efa_qkey,
      local_addr, lkey, length, remote_addr, rkey, wr_id, efa_op);
}
