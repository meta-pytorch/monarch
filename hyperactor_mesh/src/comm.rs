...
mod tests {
    ...
    /// Extract the proc rank from an ActorId. Works for both Ranked and Direct ProcIDs.
    /// For Ranked ProcIDs, the rank is directly available. For Direct ProcIDs
    /// (used by HostMesh), the rank is parsed from the proc name (e.g., "test_0-uuid").
    ...
    struct MeshSetupV1 {
        ...
        reply_tos: Vec<(PortRef<u64>, PortRef<MyReply>)>,
        // Keep the host mesh ref alive so comm actors aren't shut down.
    ...
    async fn execute_cast_and_reply_v1() {
        let setup = setup_mesh_v1::<NoneAccumulator>(None).await;
        ...
        let _ = HostMesh::take(setup.host_mesh)
            .shutdown(setup.instance)
            .await;
    }
    ...
    async fn execute_cast_and_accum_v1(config: &hyperactor_config::global::ConfigLock) {
        ...
        let setup = setup_mesh_v1(Some(accum::sum::<u64>())).await;
    ...
    async fn test_cast_and_reply_once_v1() {
        ...
        let setup = setup_once_port_mesh::<NoneAccumulator>(None).await;
    ...
    async fn test_cast_and_accum_once_v1() {
        ...
        let setup = setup_once_port_mesh(Some(accum::sum::<u64>())).await;
