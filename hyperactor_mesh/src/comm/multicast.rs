...
#[derive(Serialize, Deserialize, Debug, Clone)]
pub(super) struct DestMessageHeaders {
...
pub(crate) struct CastMessageV1 {
    ...
    /// nodes. Specifically, this includes source->comm, comm->comm, and
    /// comm->dest.
    pub(super) header_props: Attrs,
    /// The information used to set the headers of the messages sent to the
    /// destination actors. Unlike header_props, these headers will not be set
    /// on source->comm and comm->comm messages.
    pub(super) dest_headers: DestMessageHeaders,
...
impl CastMessageV1 {
    ...
    {
        let data = ErasedUnbound::try_from_message(message)?;
        let dest_headers = DestMessageHeaders {
            sender,
            session_id,
            seqs,
        };
