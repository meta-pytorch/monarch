...
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize, Named, Bind, Unbind)]
pub struct GetSubscriberCount(#[binding(include)] pub PortRef<SubscriberCount>);

/// Response for [`GetSubscriberCount`].
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize, Named, Bind, Unbind)]
pub struct SubscriberCount(pub usize);
...
impl<A: Referable> Handler<GetSubscriberCount> for ActorMeshController<A> {
    async fn handle(
        ...
        message
            .0
            .send(cx, SubscriberCount(self.health_state.subscribers.len()))?;
        Ok(())
    }
}
