...
impl<A: Referable> Actor for ActorMeshController<A> {
    async fn init(&mut self, this: &Instance<Self>) -> Result<(), anyhow::Error> {
        ...
        tracing::info!(actor = %this.self_id(), %owner, "started mesh controller for {}", self.mesh.name());
        Ok(())
    }
    ...
    async fn cleanup(
        ...
        if self.monitor.take().is_some() {
            tracing::info!(actor = %this.self_id(), mesh_name = %self.mesh.name(), "starting cleanup for ActorMeshController, stopping actor mesh");
...
impl<A: Referable> Handler<Subscribe> for ActorMeshController<A> {
    async fn handle(&mut self, cx: &Context<Self>, message: Subscribe) -> anyhow::Result<()> {
        ...
        if self.health_state.subscribers.insert(message.0) {
            tracing::debug!(actor = %cx.self_id(), num_subscribers = self.health_state.subscribers.len(), "added subscriber {} to mesh controller", port_id);
        }
        Ok(())
    }
}
...
impl<A: Referable> Handler<Unsubscribe> for ActorMeshController<A> {
    async fn handle(&mut self, cx: &Context<Self>, message: Unsubscribe) -> anyhow::Result<()> {
        if self.health_state.subscribers.remove(&message.0) {
            tracing::debug!(actor = %cx.self_id(), num_subscribers = self.health_state.subscribers.len(), "removed subscriber {} from mesh controller", message.0.port_id());
        }
        Ok(())
    }
}
...
impl<A: Referable> Handler<resource::Stop> for ActorMeshController<A> {
    async fn handle(&mut self, cx: &Context<Self>, _message: resource::Stop) -> anyhow::Result<()> {
        ...
        tracing::info!(actor = %cx.self_id(), %mesh_name, "forwarding stop request from ActorMeshController to proc mesh");
