...
declare_attrs! {
    ...
    /// owners. Default of 3 seconds is chosen to not penalize short-lived successful calls,
...
impl<A: Referable> ActorMeshController<A> {
    ...
    fn self_check_state_message(&self, cx: &Context<Self>) -> Result<(), ActorError> {
...
impl<A: Referable> Actor for ActorMeshController<A> {
    async fn init(&mut self, this: &Instance<Self>) -> Result<(), anyhow::Error> {
        ...
        let time_between_checks = hyperactor_config::global::get(SUPERVISION_POLL_FREQUENCY);
        this.self_message_with_delay(CheckState {}, time_between_checks)?;
        self.monitor = Some(());
