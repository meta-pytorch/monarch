...
impl LogTailer {
    /// Helper to convert bytes to utf8 string
    fn utf8_str_from_bytes(bytes: &[u8]) -> String {
        // RFC: usual convention is to return lossy string rather than return truncated valid utf8. Shall we do lossy?
        match std::str::from_utf8(bytes) {
            Ok(s) => s.to_string(),
            Err(e) => {
                let n = e.valid_up_to();
                if n == 0 {
                    "<INVALID UTF-8 BYTES>".to_string()
                } else {
                    std::str::from_utf8(&bytes[..n]).unwrap().to_string()
                }
            }
        }
    }
    ...
    fn push_line_to_buffer(state: &Arc<Mutex<State>>, byte_buffer: &mut [u8], max: usize) {
        let mut buffer = Self::utf8_str_from_bytes(byte_buffer);
...
mod tests {
    ...
    async fn test_truncation_with_utf8_errors() {
        ...
        assert_eq!(lines[2], "<INVALID UTF-8 BYTES>");
    }
}
