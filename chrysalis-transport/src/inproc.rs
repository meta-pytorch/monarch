...
impl DatagramSocket for InprocSocket {
    ...
    fn poll_send_ready(
        ...
        self.state.shutdown_state.register_send_waker(cx.waker());
        if !self.state.shutdown_state.is_running() {
            return Poll::Ready(Err(io::Error::new(
                io::ErrorKind::BrokenPipe,
                "in-process transport is shut down",
    ...
    fn poll_recv_from(
        ...
        self.state.shutdown_state.register_receive_waker(cx.waker());
        if !self.state.shutdown_state.is_running() {
            return Poll::Ready(Err(io::Error::new(
                io::ErrorKind::BrokenPipe,
                "in-process transport is shut down",
        ...
        let mut queue = self
            ...
            .lock()
            .expect("in-process queue lock poisoned");
        let Some(datagram) = queue.datagrams.front() else {
            if queue
                .waker
                .as_ref()
        ...
        let datagram = queue
            .datagrams
            .pop_front()
            .expect("peeked in-process datagram should remain queued");
        drop(queue);
...
mod tests {
    ...
    #[tokio::test]
    async fn undersized_receive_buffer_does_not_discard_datagram() {
        let network = Arc::new(InprocNetwork::default());
        let sender = network.bind(1).expect("bind sender");
        let receiver = network.bind(2).expect("bind receiver");
        sender
            .try_send_to(b"payload", receiver.local_addr())
            .expect("send datagram");
        let mut small = [0; 3];
        let error = receiver
            .recv_from(&mut small)
        ...
        let (length, source) = receiver
            .recv_from(&mut large)
            .await
            .expect("retry receive with sufficient space");
