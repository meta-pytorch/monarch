...
use chrysalis_transport_tokio::DuplexTransportConfig;
use chrysalis_transport_tokio::Transport;
use chrysalis_transport_tokio::TransportLimits;
...
impl SendStream {
    ...
    /// Resets this stream's send half with an application error code.
    pub fn reset(
        &self,
        error_code: u64,
    ) -> impl Future<Output = Result<(), QuicTransportError>> + Send + 'static {
        let stream = self.inner.clone();
        async move { Ok(stream.reset(error_code).await?) }
    }
...
impl AsyncWrite for SendStream {
    ...
    fn poll_shutdown(mut self: Pin<&mut Self>, context: &mut Context<'_>) -> Poll<io::Result<()>> {
        ...
        let future = self
            .pending_finish
            .as_mut()
            .expect("pending finish was initialized above");
        match future.as_mut().poll(context) {
            Poll::Pending => Poll::Pending,
            Poll::Ready(Ok(())) => {
...
impl RecvStream {
    ...
    /// Stops this stream's receive half with an application error code.
    pub fn stop(
        &self,
        error_code: u64,
    ) -> impl Future<Output = Result<(), QuicTransportError>> + Send + 'static {
        let stream = self.inner.clone();
        async move { Ok(stream.stop(error_code).await?) }
    }
}
    ...
    fn poll_read(
        ...
        loop {
            ...
            let future = self
                .pending
                .as_mut()
                .expect("pending receive was initialized above");
