        ...
        tokio::select! {
            ...
            completed = classifiers.join_next(), if !classifiers.is_empty() => {
                if let Some(Err(error)) = completed {
                    error!(error = %error, "link-local classifier task failed");
                }
            }
            incoming = transport.accept_link_local(), if classifiers.len() < classifier_capacity => {
...
async fn classify(
    ...
    .await
    {
        Ok(Ok(_)) => {}
        Ok(Err(error)) => {
            debug!(peer = ?peer, error = %error, "rejecting link-local selector read failure");
            reject(&mut incoming).await;
            return;
        }
        Err(_) => {
            debug!(peer = ?peer, "rejecting link-local selector timeout");
            reject(&mut incoming).await;
            return;
        }
    }
    ...
    let Some(delivery) = registrations.get(&id) else {
        debug!(peer = ?peer, protocol = ?id, "rejecting unknown link-local protocol");
        reject(&mut incoming).await;
        return;
    };
    let permit = match tokio::time::timeout(CLASSIFICATION_TIMEOUT, delivery.reserve()).await {
        Ok(Ok(permit)) => permit,
        Ok(Err(_)) => {
            debug!(peer = ?peer, protocol = ?id, "rejecting closed link-local protocol");
            reject(&mut incoming).await;
            return;
        }
        Err(_) => {
            debug!(peer = ?peer, protocol = ?id, "rejecting saturated link-local protocol");
            reject(&mut incoming).await;
            return;
        }
    };
