...
use chrysalis_transport_uring::ReceivedDatagram;
...
pub(crate) struct CarrierIoStats {
    ...
    transit_drops: TransitDropStats,
}

#[derive(Debug, Default)]
struct TransitDropStats {
    invalid: AtomicU64,
    unroutable: AtomicU64,
    queue_full: AtomicU64,
    withdrawn: AtomicU64,
    peer_delivery: AtomicU64,
}

#[derive(Clone, Copy, Debug)]
enum TransitDropReason {
    Invalid,
    Unroutable,
    QueueFull,
    Withdrawn,
...
enum TransitSlices {
    None,
    One(TransitSlice),
    Many(Vec<TransitSlice>),
}

impl TransitSlices {
    fn push(&mut self, slice: TransitSlice) {
        match self {
            Self::None => *self = Self::One(slice),
            Self::One(existing) if can_coalesce_transit(*existing, slice) => {
                existing.length += slice.length;
            }
            Self::One(existing) => {
                let first = *existing;
                *self = Self::Many(vec![first, slice]);
            }
            Self::Many(slices) => {
                if let Some(existing) = slices.last_mut()
                    && can_coalesce_transit(*existing, slice)
                {
                    existing.length += slice.length;
                } else {
                    slices.push(slice);
    ...
    fn for_each(self, mut use_slice: impl FnMut(TransitSlice)) {
        match self {
            Self::None => {}
            Self::One(slice) => use_slice(slice),
            Self::Many(slices) => slices.into_iter().for_each(use_slice),
        }
    }
}

fn can_coalesce_transit(left: TransitSlice, right: TransitSlice) -> bool {
    left.target == right.target
        && left.segment_size == right.segment_size
        && left.offset.checked_add(left.length) == Some(right.offset)
}
...
impl RoutedUdpPacketIo {
    ...
    fn record_transit_drop(&self, length: usize, segment_size: usize, reason: TransitDropReason) {
        record_transit_drop(self.fallback_stats.as_ref(), length, segment_size, reason);
    }
    ...
    fn enqueue_transit(
        ...
        let Some(end) = offset.checked_add(length).filter(|end| *end <= bytes.len()) else {
            self.record_transit_drop(length, segment_size, TransitDropReason::Invalid);
        ...
        let Some(route) = self
            ...
            self.record_transit_drop(length, segment_size, TransitDropReason::Unroutable);
            self.recycle_transit_buffer(bytes);
            return;
        };
        if route.destination().scheme() == "udp" {
            ...
            {
                self.record_transit_drop(length, segment_size, TransitDropReason::QueueFull);
                self.recycle_transit_buffer(bytes);
                return;
            }
    ...
    fn forward_received_transit(&mut self, datagram: ReceivedDatagram, transit: TransitSlice) {
        let Some(route) = self
            .router
            .get(transit.target)
            .or_else(|| self.router.default_route())
        else {
            self.record_transit_drop(
                transit.length,
                transit.segment_size,
                TransitDropReason::Unroutable,
            );
            return;
        };
        if route.destination().scheme() == "udp" {
            if transit.segment_size > self.segment_size {
                self.record_transit_drop(
                    transit.length,
                    transit.segment_size,
                    TransitDropReason::Invalid,
                );
                return;
            }
            if transit.segment_size == self.segment_size || transit.length <= transit.segment_size {
                let result = route.try_with_destination(|destination| {
                    self.udp.try_forward_received(
                        datagram,
                        transit.offset..transit.offset + transit.length,
                        decode_udp_addr(destination)?,
                        Instant::now(),
                    )
                });
                match result {
        ...
        let Some(end) = transit
            .offset
            .checked_add(transit.length)
        ...
        bytes.clear();
        bytes.extend_from_slice(&self.udp.received(datagram)[transit.offset..end]);
            ...
            match result {
                ...
                Ok(None) => {
                    let pending = self
                        .transit_transmits
                        .pop_front()
                        .expect("withdrawn transit remains queued");
                    self.record_transit_drop(
                        pending.length,
                        pending.segment_size,
                        TransitDropReason::Withdrawn,
                ...
                Err(error) if is_peer_delivery_error(&error) => {
                    ...
                    self.record_transit_drop(
                        pending.length,
                        pending.segment_size,
                        TransitDropReason::PeerDelivery,
                    );
                    self.recycle_transit_buffer(pending.bytes);
                }
...
struct RoutedUdpSendSlot<'a> {
    ...
    free_transit_buffers: &'a mut Vec<Vec<u8>>,
    fallback_stats: &'a CarrierIoStats,
    segment_size: usize,
}
...
impl PacketSendSlot for RoutedUdpSendSlot<'_> {
    ...
    fn submit(
        ...
        if destination.address().scheme() == "udp" {
            let submitted = destination.try_with_address(|address| {
        ...
        if self.fallback_transmits.len() >= TRANSIT_QUEUE_CAPACITY {
            record_transit_drop(
                ...
                length,
                segment_size,
                TransitDropReason::QueueFull,
            );
            return Ok(());
        }
        let mut bytes = self.free_transit_buffers.pop().unwrap_or_default();
        bytes.clear();
        bytes.extend_from_slice(&slot.buffer_mut()[..length]);
...
impl PacketIo for RoutedUdpPacketIo {
    ...
    fn try_send_slot(&mut self) -> Option<Box<dyn PacketSendSlot + '_>> {
        ...
        let free_transit_buffers = &mut self.free_transit_buffers;
    ...
    fn drain_received(
        ...
        while let Some(event) = self.udp.try_next_event() {
            ...
            let (result, transit) = if length == 0 {
                (Ok(()), TransitSlices::None)
            } else {
                let source_address = crate::UdpSocket::datagram_addr(datagram.source());
                let addresses = self.addresses.clone();
                let mut registered_source = None;
                let mut source = || {
                    *registered_source
                        .get_or_insert_with(|| addresses.register(source_address.clone()))
                };
                let aggregate = self.udp.received(datagram);
                classify_received_aggregate(
                    self.local_pid,
            ...
            transit.for_each(|transit| {
                self.forward_received_transit(datagram, transit);
            });
            self.udp.release_receive(datagram.slot());
            result?;
        }
        while let Some(mut aggregate) = self.fallback_received.pop_front() {
            let aggregate_source = aggregate.source;
...
fn classify_received_aggregate(
    ...
    bytes: &mut [u8],
    segment_size: usize,
    source: &mut dyn FnMut() -> SocketAddr,
    local: SocketAddr,
    receive: &mut dyn FnMut(&mut [u8], SocketAddr, SocketAddr) -> Result<(), Error>,
) -> (Result<(), Error>, TransitSlices) {
    assert!(segment_size > 0, "received segment size must be nonzero");
    let mut transit = TransitSlices::None;
    let mut result = Ok(());
...
fn record_transit_drop(
    stats: &CarrierIoStats,
    length: usize,
    segment_size: usize,
    reason: TransitDropReason,
) {
    let datagrams = length.div_ceil(segment_size.max(1)).max(1);
    let counter = match reason {
        TransitDropReason::Invalid => &stats.transit_drops.invalid,
        TransitDropReason::Unroutable => &stats.transit_drops.unroutable,
        TransitDropReason::QueueFull => &stats.transit_drops.queue_full,
        TransitDropReason::Withdrawn => &stats.transit_drops.withdrawn,
        TransitDropReason::PeerDelivery => &stats.transit_drops.peer_delivery,
    };
...
impl<T: DatagramSocket> CarrierPacketIo<T> {
    ...
    fn flush_transmits(&mut self, context: &mut Context<'_>) -> io::Result<()> {
        loop {
            ...
            let segment_size = (remaining > self.segment_size).then_some(self.segment_size);
            let transmit = DatagramTransmit {
            ...
            match self.socket.try_send_transmit(&transmit) {
                ...
                Ok(accepted) => {
                    ...
                    if accepted == segment_count {
                        let pending = self
                            .pending_transmit
                            .pop_front()
                            .expect("accepted transmit should remain queued");
                ...
                Err(error) if is_peer_delivery_error(&error) => {
                    ...
                    let pending = self
                        .pending_transmit
                        .pop_front()
                        .expect("failed transmit should remain queued");
                    self.free_transmit.push(pending.buffer);
                }
                Err(error) => return Err(error),
            }
...
mod tests {
    ...
    #[test]
    fn unroutable_transit_is_counted_and_recycled() {
        let config = config();
        let driver =
            UdpDriver::new(socket(), config.udp_driver_config()).expect("create forwarder driver");
        let (mut forwarder, _, _) =
            RoutedUdpPacketIo::new(driver, SOURCE, Arc::new(Router::new()), None);
        let payload = payload_segments(TARGET, 3);
        ...
        assert_eq!(
            ...
            3,
            "all unroutable datagrams should be counted"
        );
        assert_eq!(
            forwarder.free_transit_buffers.len(),
            1,
            "the rejected aggregate should return to the pool"
        );
    }
    ...
    fn routed_udp_classifies_mixed_aggregate_without_copying_packets() {
        ...
        let TransitSlices::One(transit) = transit else {
            panic!("expected one transit range");
        };
        assert_eq!(transit.offset, SEGMENT_SIZE);
        assert_eq!(transit.length, SEGMENT_SIZE);
        assert_eq!(transit.target, TARGET);
    }
    ...
    fn routed_udp_forwards_nonlocal_cids_inside_the_completion_owner() {
        ...
        let (mut forwarder, forwarder_addresses, forwarder_stats) =
        ...
        assert!(
            forwarder.free_transit_buffers.is_empty(),
            "available UDP slots should receive transit bytes directly"
        );
        assert!(
            forwarder_addresses
                .inner
                ...
                .by_address
                .is_empty(),
            "transit-only UDP sources should not enter the address book"
        );
    }
}
