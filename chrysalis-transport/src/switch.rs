...
impl<T: DatagramSocket> DatagramSwitch<T> {
    ...
    pub fn spawn(carrier: T, router: Arc<Router>) -> Self {
    ...
    fn bind_with_options(
        ...
        Ok(SwitchSocket {
            ...
            receiver: Mutex::new(BindingReceiver {
                ...
                readiness: shutdown_state.clone(),
            }),
            shutdown_state,
        })
    }
...
impl DatagramSocket for SwitchSocket {
    ...
    fn try_send_transmit(&self, transmit: &DatagramTransmit<'_>) -> io::Result<usize> {
        ...
        if self.route_outbound {
            if transmit_target(transmit).is_some_and(Pid::is_link_local) {
    ...
    fn poll_send_ready(
        ...
        if self.route_outbound {
            let Some(target) = transmit_target(transmit) else {
                return Poll::Ready(Ok(()));
            };
            if target.is_link_local() {
                return self.switch.egress.poll_send_ready(cx, transmit);
            }
            return self.switch.poll_send_ready(cx, target, transmit);
        }
        let Some((switch, target)) = parse_switched_addr(transmit.destination)? else {
            return self.switch.egress.poll_send_ready(cx, transmit);
        };
        if switch != self.switch.id || transmit_target(transmit) != Some(target) {
            return Poll::Ready(Ok(()));
        }
        self.switch.poll_send_ready(cx, target, transmit)
    }
    ...
    fn poll_recv_from(
        ...
        let first_len = datagram.bytes.len().min(datagram.stride);
        if first_len > buffer.remaining() {
            receiver.pending.push_front(datagram);
...
impl BindingReceiver {
    fn poll_recv(&mut self, cx: &mut Context<'_>) -> Poll<Option<SwitchedDatagram>> {
        match self.pending.pop_front() {
            ...
            None => match self.incoming.poll_recv(cx) {
                Poll::Ready(Some(datagram)) => {
                    self.readiness.wake_send();
                    Poll::Ready(Some(datagram))
                }
                result => result,
            },
    ...
    fn try_recv(&mut self) -> Option<SwitchedDatagram> {
        if let Some(datagram) = self.pending.pop_front() {
            return Some(datagram);
        }
        let datagram = self.incoming.try_recv().ok()?;
        self.readiness.wake_send();
        Some(datagram)
    }
}
...
impl SwitchState {
    fn poll_send_ready(
        ...
        if let Some(local) = local
            && local.shutdown_state.is_running()
        {
            if local.sender.capacity() != 0 {
                return Poll::Ready(Ok(()));
            }
            local.shutdown_state.register_send_waker(cx.waker());
            if !local.shutdown_state.is_running() || local.sender.capacity() != 0 {
                return Poll::Ready(Ok(()));
            }
            return Poll::Pending;
        }
        self.router
            .poll_send_ready(cx, self.egress.as_ref(), target, transmit)
    }
...
async fn run_switch<T: DatagramSocket>(
    ...
    let mut regrouped = Vec::with_capacity(MAX_DATAGRAM_SIZE * DATAGRAM_BATCH_SIZE);
    loop {
        ...
        while index < count {
            ...
            if received.stride < received.len {
                observe_transmit(state.dispatch_transmit(
                    &DatagramTransmit {
            ...
            let Some(target) = target_pid(datagram) else {
                observe_dispatch(state.dispatch(datagram, received.source.clone()));
                index += 1;
                continue;
            };
...
fn observe_dispatch(result: io::Result<ForwardDisposition>) {
    match result {
        Ok(ForwardDisposition::Forwarded { .. }) => {}
        Ok(ForwardDisposition::Dropped(reason)) => {
            tracing::debug!(?reason, "datagram switch dropped an inbound datagram");
        }
        Err(error) => {
            tracing::warn!(%error, "datagram switch failed to dispatch an inbound datagram");
        }
    }
}

fn observe_transmit(result: io::Result<(ForwardDisposition, usize)>) {
    observe_dispatch(result.map(|(disposition, _)| disposition));
}

fn transmit_target(transmit: &DatagramTransmit<'_>) -> Option<Pid> {
    let first_len = transmit
        .segment_size
        .unwrap_or(transmit.contents.len())
        .min(transmit.contents.len());
    target_pid(&transmit.contents[..first_len])
}
...
mod tests {
    ...
    async fn forwards_nonlocal_datagrams_through_router() {
        ...
        let switch_carrier = network.bind(2).expect("bind switch");
    ...
    async fn local_switch_sockets_exchange_datagrams_without_egress() {
        let network = InprocNetwork::new(NonZeroUsize::new(32).expect("nonzero capacity"));
        let carrier = network.bind(1).expect("bind switch carrier");
    ...
    #[tokio::test]
    async fn undersized_single_receive_retains_segmented_batch() {
        let network = InprocNetwork::new(NonZeroUsize::new(32).expect("nonzero capacity"));
        let carrier = network.bind(1).expect("bind switch carrier");
        let datagram_switch = DatagramSwitch::spawn(carrier, Arc::new(Router::new()));
        let source = datagram_switch.bind(SOURCE).expect("bind source PID");
        ...
        let mut undersized = [0; 32];
        assert_eq!(
            destination
                .recv_from(&mut undersized)
                .await
                .expect_err("reject undersized receive buffer")
                .kind(),
            io::ErrorKind::InvalidData
        );
        let mut buffer = [0; 64];
        for _ in 0..2 {
            let (len, _) = destination
                .recv_from(&mut buffer)
                .await
                .expect("receive retained datagram");
            assert_eq!(&buffer[..len], segment);
        }
    ...
    #[tokio::test]
    async fn undersized_batch_receive_retains_datagram() {
        let network = InprocNetwork::new(NonZeroUsize::new(32).expect("nonzero capacity"));
        let carrier = network.bind(1).expect("bind switch carrier");
        let datagram_switch = DatagramSwitch::spawn(carrier, Arc::new(Router::new()));
        let source = datagram_switch.bind(SOURCE).expect("bind source PID");
        ...
        source
            ...
            .expect("send local datagram");
        let mut undersized = [0; 32];
        let mut buffers = [IoSliceMut::new(&mut undersized)];
        let mut meta = [DatagramRecvMeta::default()];
        assert_eq!(
            poll_fn(|cx| destination.poll_recv(cx, &mut buffers, &mut meta))
                .await
                .expect_err("reject undersized batch receive buffer")
                .kind(),
            io::ErrorKind::InvalidData
        );
    ...
    async fn routed_binding_uses_cid_instead_of_supplied_address() {
        let network = InprocNetwork::new(NonZeroUsize::new(32).expect("nonzero capacity"));
        let switch_carrier = network.bind(1).expect("bind switch carrier");
    ...
    #[tokio::test]
    async fn routed_binding_recognizes_short_final_link_local_segment() {
        let network = InprocNetwork::new(NonZeroUsize::new(32).expect("nonzero capacity"));
        let carrier = network.bind(1).expect("bind switch carrier");
        let adjacent = network.bind(2).expect("bind adjacent endpoint");
        let datagram_switch = DatagramSwitch::spawn(carrier, Arc::new(Router::new()));
        let source = datagram_switch
            .bind_routed(SOURCE)
            .expect("bind routed source PID");
        let expected = datagram(Pid::LINK_LOCAL);

        assert_eq!(
            source
                .try_send_transmit(&DatagramTransmit {
                    destination: adjacent.local_addr(),
                    contents: &expected,
                    segment_size: Some(expected.len() + 1),
                    ecn: None,
                    source_ip: None,
                })
                .expect("send short final link-local segment"),
            1
        );
        let mut buffer = [0; 64];
        let (length, _) = adjacent
            .recv_from(&mut buffer)
            .await
            .expect("receive link-local segment at supplied destination");
    ...
    #[tokio::test]
    async fn send_readiness_follows_full_local_binding() {
        let network = InprocNetwork::new(NonZeroUsize::new(32).expect("nonzero capacity"));
        let carrier = network.bind(1).expect("bind switch carrier");
        let datagram_switch = DatagramSwitch::spawn(carrier, Arc::new(Router::new()));
        let source = datagram_switch.bind(SOURCE).expect("bind source PID");
        let destination = datagram_switch
            .bind_with_queue_capacity(
                TARGET,
                NonZeroUsize::new(1).expect("test queue capacity is nonzero"),
            )
            .expect("bind destination PID");
        let expected = datagram(TARGET);
        ...
        assert_eq!(source.try_send_transmit(&transmit).expect("fill queue"), 1);
        assert!(
            timeout(
                Duration::from_millis(10),
                poll_fn(|cx| source.poll_send_ready(cx, &transmit)),
            )
            .await
            .is_err(),
            "full local queue should keep send readiness pending"
        );
        let mut buffer = [0; 64];
        destination
            .recv_from(&mut buffer)
            .await
            .expect("drain local destination");
        timeout(
            Duration::from_secs(1),
            poll_fn(|cx| source.poll_send_ready(cx, &transmit)),
        )
        .await
        .expect("local readiness wake timed out")
        .expect("local destination should become writable");
    ...
    #[tokio::test]
    async fn routed_send_readiness_follows_selected_destination() {
        let network = InprocNetwork::new(NonZeroUsize::new(1).expect("nonzero capacity"));
        let carrier = network.bind(1).expect("bind switch carrier");
        let destination = network.bind(2).expect("bind routed destination");
        let unrelated = network.bind(3).expect("bind unrelated destination");
        let filler = network.bind(4).expect("bind filler source");
        let router = Arc::new(Router::new());
        router.insert(TARGET, Route::permanent(destination.local_addr().clone()));
        let datagram_switch = DatagramSwitch::spawn(carrier, router);
        let source = datagram_switch
            .bind_routed(SOURCE)
            .expect("bind routed source PID");
        let expected = datagram(TARGET);
        filler
            .try_send_to(b"full", destination.local_addr())
            .expect("fill routed destination");
        let transmit = DatagramTransmit {
            destination: unrelated.local_addr(),
            contents: &expected,
            segment_size: None,
            ecn: None,
            source_ip: None,
        ...
        assert!(
            ...
            .await
            .is_err(),
            "readiness should follow the full routed destination"
        );
        let mut buffer = [0; 64];
        destination
            .recv_from(&mut buffer)
            .await
            .expect("drain routed destination");
        timeout(
            Duration::from_secs(1),
            poll_fn(|cx| source.poll_send_ready(cx, &transmit)),
        )
        .await
        .expect("routed readiness wake timed out")
        .expect("routed destination should become writable");
