    ...
    fn try_send_transmit(&self, transmit: &DatagramTransmit<'_>) -> io::Result<usize> {
        ...
        if transmit.contents.is_empty() {
            return match self.socket().try_send_to(&[], destination) {
                Ok(0) => Ok(1),
                Ok(_) => unreachable!("empty UDP send wrote bytes"),
...
mod tests {
    ...
    async fn zero_length_datagram_is_transmitted() {
        ...
        let receiver = UdpSocket::bind(SocketAddr::from((Ipv4Addr::LOCALHOST, 0)))
            .await
            .expect("bind receiver UDP socket");
        ...
        poll_fn(|cx| sender.poll_send_ready(cx, &transmit))
            .await
            .expect("wait for empty UDP send readiness");
