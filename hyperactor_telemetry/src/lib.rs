...
pub trait TelemetryTestHandle {
    ...
    fn get_execution_samples(&self) -> Vec<TelemetrySample>;
}

#[cfg(fbcode_build)]
struct MockScubaHandle {
    tracing_client: crate::meta::scuba_utils::MockScubaClient,
    execution_client: crate::meta::scuba_utils::MockScubaClient,
}
...
fn initialize_logging_with_log_prefix_impl(
    ...
    #[cfg(fbcode_build)]
    {
        ...
        let mut mock_scuba_clients: Option<(
        ...
        if use_unified {
...

            {
                if !is_layer_disabled(DISABLE_OTEL_TRACING) {
                    use crate::meta;
                    use crate::meta::get_execution_targets;
                    ...
                    if mock_scuba {
                        ...
                        let executions_client = meta::scuba_utils::MockScubaClient::new();

                        let tracing_exporter = meta::scuba_exporter::ScubaExporter::with_client(
                        ...
                        exporters.push(Box::new(tracing_exporter));

                        let executions_exporter = meta::scuba_exporter::ScubaExporter::with_client(
                            executions_client.clone(),
                            match meta::executions_resource().get(&LOG_ENTER_EXIT) {
                                Some(Value::Bool(enabled)) => enabled,
                                _ => false,
                            },
                        )
                        .with_target_filter(get_execution_targets());
                        exporters.push(Box::new(executions_exporter));

                        mock_scuba_clients = Some((tracing_client, executions_client));
                    } else {
                        let tracing_exporter =
                            meta::scuba_exporter::ScubaExporter::new(meta::tracing_resource())
                                .with_target_filter(get_tracing_targets());
                        exporters.push(Box::new(tracing_exporter));

                        let executions_exporter =
                            meta::scuba_exporter::ScubaExporter::new(meta::executions_resource())
                                .with_target_filter(get_execution_targets());
                        exporters.push(Box::new(executions_exporter));
                    }
                }
            ...
            if mock_scuba {
                ...
                let executions_client = crate::meta::scuba_utils::MockScubaClient::new();

                let scuba_layer = crate::meta::tracing_layer_with_clients(
                    tracing_client.clone(),
                    executions_client.clone(),
                );
        ...
        if let Some((tracing_client, execution_client)) = mock_scuba_clients {
            Box::new(MockScubaHandle {
                tracing_client,
                execution_client,
            })
        } else {
