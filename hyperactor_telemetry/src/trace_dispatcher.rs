...
impl TraceEventDispatcher {
    ...
    /// Uses a bounded channel (capacity 10,000) to ensure telemetry never blocks
    ...
    pub(crate) fn new(
        ...
        let worker_handle = std::thread::Builder::new()
            ...
            .spawn(move || {
                worker_loop(receiver, sinks, dropped_events_worker);
            })
        ...
        if let Some(sender) = &self.sender {
            if let Err(mpsc::TrySendError::Full(_)) = sender.try_send(event) {
                ...
                if dropped.is_multiple_of(1000) {
                    eprintln!(
                        "[telemetry] WARNING: {} events dropped due to full queue (capacity: 10,000). \
                        Telemetry worker may be falling behind.",
                        dropped
                    );
                }
            }
...
impl Drop for TraceEventDispatcher {
    fn drop(&mut self) {
        ...
        // will run it's own drop impl to join the thread and flush
...
/// Background worker loop that receives events and dispatches them to sinks.
